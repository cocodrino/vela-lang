# VELA Base Lexicon — 1,000+ Words

> **Status:** Phase 4 — IN PROGRESS  
> **Words:** ~400 complete in this draft  
> **Philosophy:** English primary | Latin when universally clear | Transparent compounds preferred  
> **Format:** Word → AFI → Category → English → Example sentence

---

## Section 1: Core — Pronouns, Articles, Small Words

*These are the most frequent words in any language. Short, transparent, memorized naturally.*

### Pronouns

| Word | AFI | Type | English | VELA Example | English |
|------|-----|------|---------|-------------|---------|
| **mi** | /mi/ | pronoun | I | Mi go a la siti. | I go to the city. |
| **yu** | /ju/ | pronoun | you (singular) | Yu si la man. | You see the man. |
| **li** | /li/ | pronoun | he/she/it | Li es hapi. | She is happy. |
| **wi** | /wi/ | pronoun | we | Wi liv la siti-te. | We live in the city. |
| **de** | /de/ | pronoun | they | De wok la ofis-te. | They work at the office. |
| **self** | /self/ | reflexive | self | Mi si self. | I see myself. |

### Possessive Short Form (pronoun + **-f**)

| Word | AFI | English | Example | English |
|------|-----|---------|---------|---------|
| **mif** | /mif/ | my | Mi si mif fren. | I see my friend. |
| **yuf** | /juf/ | your | Yu si yuf buk. | You see your book. |
| **liz** | /liz/ | his/her/its | Li her liz vois. | She hears her voice. |
| **wef** | /wef/ | our | Wi liv wef hous. | We live in our house. |
| **def** | /def/ | their | De liv def siti. | They live in their city. |

### Demonstratives

| Word | AFI | English | Example | English |
|------|-----|---------|---------|---------|
| **dis** | /dis/ | this | Dis man es big. | This man is big. |
| **dat** | /dat/ | that | Dat hous es nais. | That house is nice. |
| **dese** | /de.se/ | these | Dese man-s es strong. | These men are strong. |
| **dase** | /da.se/ | those | Dase buk-s es mi-se. | Those books are mine. |

### Articles

| Word | AFI | English | Example | English |
|------|-----|---------|---------|---------|
| **la** | /la/ | the (definite) | La man si la film. | The man sees the film. |
| **un** | /un/ | a/an (indefinite) | Un man kom. | A man comes. |

### Essential Prepositions

| Word | AFI | English | Example | English |
|------|-----|---------|---------|---------|
| **a** | /a/ | to / at | Mi go a la ofis. | I go to the office. |
| **in** | /in/ | in | Mi liv in la siti. | I live in the city. |
| **on** | /on/ | on | La buk es on la tebul. | The book is on the table. |
| **from** | /from/ | from | Wi kom from la skol. | We come from the school. |
| **for** | /for/ | for | Dis es for yu. | This is for you. |
| **wit** | /wit/ | with | Mi wok wit yu. | I work with you. |
| **at** | /at/ | at | Wi wok at la sentr. | We work at the center. |
| **of** | /of/ | of | La ofis of la siti. | The office of the city. |
| **bifor** | /bifor/ | before | Mi wok bifor yu kom. | I work before you come. |
| **aft** | /aft/ | after | Wi kom aft la wok. | We come after the work. |
| **arond** | /arond/ | around | Wi wok arond la siti. | We work around the city. |
| **between** | /between/ | between | La hous es between tu tri-s. | The house is between three. |
| **insaid** | /insaid/ | inside | La buk es insaid la box. | The book is inside the box. |
| **autsid** | /autsid/ | outside | Li liv autsid la siti. | She lives outside the city. |
| **neer** | /neer/ | near | La skol es neer la park. | The school is near the park. |
| **far** | /far/ | far | La siti es far from hier. | The city is far from here. |

### Conjunctions

| Word | AFI | English | Example | English |
|------|-----|---------|---------|---------|
| **and** | /and/ | and | Mi si la man and la wuman. | I see the man and the woman. |
| **or** | /or/ | or | Yu wan kom or no? | Do you want to come or not? |
| **bot** | /bot/ | but | Mi wok, bot mi es hapi. | I work, but I am happy. |
| **bikos** | /bikos/ | because | Mi wok bikos mi wan moni. | I work because I want money. |
| **if** | /if/ | if | If yu kom, den mi es hapi. | If you come, I am happy. |
| **wen** | /wen/ | when | Wen yu kom, mi si yu. | When you come, I see you. |
| **den** | /den/ | then | If yu no kom, den mi go. | If you don't come, then I go. |
| **so** | /so/ | so / therefore | Mi es hapi, so mi tok. | I am happy, so I speak. |
| **tuo** | /tuo/ | although | Mi wok tuo mi es tri. | I work although I am tired. |

### Question Words

| Word | AFI | English | Example | English |
|------|-----|---------|---------|---------|
| **hu** | /hu/ | who | Hu yu si? | Who do you see? |
| **wat** | /wat/ | what | Wat yu lik? | What do you like? |
| **wen** | /wen/ | when | Wen de kom? | When do they come? |
| **wer** | /wer/ | where | Wer wi wok? | Where do we work? |
| **hai** | /hai/ | how | Hai yu fel? | How do you feel? |
| **wai** | /wai/ | why | Wai yu wok? | Why do you work? |
| **hou** | /hou/ | how much | Hou big es la hous? | How big is the house? |
| **wot** | /wot/ | which | Wot kolor es la kar? | Which color is the car? |

### Answers

| Word | AFI | English | Use |
|------|-----|---------|-----|
| **ye** | /je/ | yes | affirmative |
| **no** | /no/ | no | negative answer |
| **mi nof no** | /mi nof no/ | I don't know | uncertainty |

### Time Words

| Word | AFI | English | Example | English |
|------|-----|---------|---------|---------|
| **nau** | /nau/ | now | Mi wok nau. | I work now. |
| **den** | /den/ | then | Wi kom den. | We come then. |
| **nau-dei** | /nau.dei/ | today | Nau-dei mi wok. | Today I work. |
| **tomoro** | /tomoro/ | tomorrow | Tomoro wi kom. | Tomorrow we come. |
| **yestdei** | /jestdei/ | yesterday | Yestdei mi go a la siti. | Yesterday I went to the city. |
| **oldei** | /oldei/ | always | Mi es hapi oldei. | I am always happy. |
| **nevr** | /nevr/ | never | Li nevr kom. | She never comes. |
| **ofn** | /ofn/ | often | Mi wok ofn. | I work often. |
| **somtaim** | /somtaim/ | sometimes | Wi kom somtaim. | We come sometimes. |
| **rarli** | /rar.li/ | rarely | Li wok rarli. | She rarely works. |
| **sun** | /sun/ | soon | Wi kom sun. | We come soon. |
| **let** | /let/ | late | Mi kom let. | I come late. |
| **erli** | /er.li/ | early | Mi wok erli. | I work early. |
| **mor** | /mor/ | more | Mi wan mor. | I want more. |
| **les** | /les/ | less | Mi hav les. | I have less. |
| **lesti** | /les.ti/ | at least | Lest wan dei. | At least one day. |
| **agen** | /agen/ | again | Mi kom agen. | I come again. |

### Days of the Week

| Word | AFI | English |
|------|-----|---------|
| **Mondei** | /mon.dei/ | Monday |
| **Tiuzdei** | /tius.dei/ | Tuesday |
| **Wenzdei** | /wens.dei/ | Wednesday |
| **Terzdei** | /ters.dei/ | Thursday |
| **Fraidei** | /frai.dei/ | Friday |
| **Satrdei** | /satrs.dei/ | Saturday |
| **Sandei** | /san.dei/ | Sunday |

### Months

| Word | AFI | English |
|------|-----|---------|
| **Januari** | /janua.ri/ | January |
| **Februari** | /februa.ri/ | February |
| **Marti** | /mar.ti/ | March |
| **Aprim** | /aprim/ | April |
| **Maji** | /maji/ | May |
| **Juni** | /juni/ | June |
| **Julai** | /julai/ | July |
| **Agost** | /agost/ | August |
| **Septembr** | /septembr/ | September |
| **Octobr** | /oktobr/ | October |
| **Novembr** | /novembr/ | November |
| **Decembr** | /decembr/ | December |

---

## Section 2: Core Verbs — Zero Irregular

*Every verb follows the same 3-suffix rule. Present -a | Past -ed | Future -wil*

### To Be and Essential States

| Word | AFI | English | Present | Past | Future | Example |
|------|-----|---------|---------|------|--------|---------|
| **bi** | /bi/ | to be | bi-a | bi-ed | bi-wil | Mi bi-a hapi. → I am happy. |
| **es** | /es/ | to be (stative) | es-a | es-ed | es-wil | La man es-a big. → The man is big. |
| **nof** | /nof/ | to know | nof-a | nof-ed | nof-wil | Mi nof wat yu wan. → I know what you want. |
| **nof-hu** | /nof.hu/ | to know someone | nof-hu-a | nof-hu-ed | nof-hu-wil | Mi nof-hu yu. → I know you. |

### Perception and Communication

| Word | AFI | English | Present | Past | Future | Example |
|------|-----|---------|---------|------|--------|---------|
| **si** | /si/ | to see | si-a | si-ed | si-wil | Mi si la film. → I see the film. |
| **her** | /her/ | to hear | her-a | her-ed | her-wil | Mi her la sonj. → I hear the song. |
| **smel** | /smel/ | to smell | smel-a | smel-ed | smel-wil | Mi smel la flauor. → I smell the flower. |
| **tas** | /tas/ | to taste | tas-a | tas-ed | tas-wil | La supa tas-a gud. → The soup tastes good. |
| **fel** | /fel/ | to feel | fel-a | fel-ed | fel-wil | Mi fel hapi. → I feel happy. |
| **luk** | /luk/ | to look | luk-a | luk-ed | luk-wil | Mi luk at la sinti. → I look at the scene. |
| **lisen** | /lisen/ | to listen | lisen-a | lisen-ed | lisen-wil | Wi lisen a la muzik. → We listen to the music. |
| **tok** | /tok/ | to speak | tok-a | tok-ed | tok-wil | Mi tok wit yu. → I speak with you. |
| **tal** | /tal/ | to tell | tal-a | tal-ed | tal-wil | Yu tal mi wat yunof. → You tell me what you know. |
| **ask** | /ask/ | to ask | ask-a | ask-ed | ask-wil | Mi ask yu wan ting. → I ask you one thing. |
| **ansr** | /ansr/ | to answer | ansr-a | ansr-ed | ansr-wil | Li ansr mi-se kwestion. → She answers my question. |

### Movement

| Word | AFI | English | Present | Past | Future | Example |
|------|-----|---------|---------|------|--------|---------|
| **go** | /go/ | to go | go-a | go-ed | go-wil | Mi go a la park. → I go to the park. |
| **kom** | /kom/ | to come | kom-a | kom-ed | kom-wil | Yu kom a mi. → You come to me. |
| **kom-hom** | /kom.hom/ | to return home | kom-hom-a | kom-hom-ed | kom-hom-wil | Mi kom-hom nau. → I come home now. |
| **liv** | /liv/ | to live | liv-a | liv-ed | liv-wil | Wi liv la siti-te. → We live in the city. |
| **wok** | /wok/ | to walk | wok-a | wok-ed | wok-wil | Mi wok la parke-te. → I walk in the park. |
| **rul** | /rul/ | to travel | rul-a | rul-ed | rul-wil | Wi rul tu la naid. → We travel to the nation. |
| **ran** | /ran/ | to run | ran-a | ran-ed | ran-wil | La man ran. → The man runs. |
| **jamp** | /jamp/ | to jump | jamp-a | jamp-ed | jamp-wil | Li jamp over la bond. → She jumps over the bond. |
| **swim** | /swim/ | to swim | swim-a | swim-ed | swim-wil | Wi swim in la rivr. → We swim in the river. |
| **frai** | /frai/ | to fly | frai-a | frai-ed | frai-wil | La berd frai. → The bird flies. |
| **folo** | /folo/ | to follow | folo-a | folo-ed | folo-wil | Mi folo yu. → I follow you. |
| **les** | /les/ | to leave | les-a | les-ed | les-wil | Mi les la hous. → I leave the house. |
| **ariv** | /ariv/ | to arrive | ariv-a | ariv-ed | ariv-wil | Wi ariv at sevn. → We arrive at seven. |

### Actions and Work

| Word | AFI | English | Present | Past | Future | Example |
|------|-----|---------|---------|------|--------|---------|
| **don** | /don/ | to do | don-a | don-ed | don-wil | Mi don la wok. → I do the work. |
| **mak** | /mak/ | to make | mak-a | mak-ed | mak-wil | Mi mak la disf. → I make the dinner. |
| **wok** | /wok/ | to work | wok-a | wok-ed | wok-wil | Wi wok in la ofis. → We work in the office. |
| **lern** | /lern/ | to learn | lern-a | lern-ed | lern-wil | Yu lern VELA. → You learn VELA. |
| **teach** | /tiatc/ | to teach | teach-a | teach-ed | teach-wil | Mi teach yu. → I teach you. |
| **stodi** | /stodi/ | to study | stodi-a | stodi-ed | stodi-wil | Mi stodi evri dei. → I study every day. |
| **reserch** | /risertc/ | to research | reserch-a | reserch-ed | reserch-wil | Li reserch la sindei. → She researches the subject. |
| **plan** | /plan/ | to plan | plan-a | plan-ed | plan-wil | Wi plan tu kom. → We plan to come. |
| **build** | /bild/ | to build | build-a | build-ed | build-wil | De build un hous. → They build a house. |
| **fix** | /fiks/ | to fix | fix-a | fix-ed | fix-wil | Mi fix la masin. → I fix the machine. |
| **brak** | /brak/ | to break | brak-a | brak-ed | brak-wil | Li brak la glas. → She breaks the glass. |
| **opn** | /opn/ | to open | opn-a | opn-ed | opn-wil | Mi opn la dor. → I open the door. |
| **clos** | /klos/ | to close | klos-a | klos-ed | klos-wil | Yu klos la windo. → You close the window. |
| **klian** | /klian/ | to clean | klian-a | klian-ed | klian-wil | Wi klian la hous. → We clean the house. |
| **kuk** | /kuk/ | to cook | kuk-a | kuk-ed | kuk-wil | Mi kuk evriSandEI. → I cook every Sunday. |
| **wosh** | /wos/ | to wash | wosh-a | wosh-ed | wosh-wil | Mi wosh mi-se masin. → I wash my car. |
| **drai** | /drai/ | to dry | drai-a | drai-ed | drai-wil | Mi drai la plet. → I dry the plate. |
| **siv** | /siv/ | to sieve / to filter | siv-a | siv-ed | siv-wil | Sift flour from solit-partikl-s. → Sieve flour from solid particles. |

### Wants, Needs, and Preferences

| Word | AFI | English | Present | Past | Future | Example |
|------|-----|---------|---------|------|--------|---------|
| **wan** | /wan/ | to want | wan-a | wan-ed | wan-wil | Mi wan kom. → I want to come. |
| **need** | /nid/ | to need | need-a | need-ed | need-wil | Mi need hel. → I need help. |
| **lik** | /lik/ | to like | lik-a | lik-ed | lik-wil | Mi lik la sonj. → I like the song. |
| **dislik** | /dislik/ | to dislike | dislik-a | dislik-ed | dislik-wil | Mi dislik la test. → I dislike the taste. |
| **prefer** | /prefer/ | to prefer | prefer-a | prefer-ed | prefer-wil | Mi prefer te rather than kofi. → I prefer tea rather than coffee. |
| **hop** | /hop/ | to hope | hop-a | hop-ed | hop-wil | Mi hop yu kom. → I hope you come. |
| **min** | /min/ | to mean | min-a | min-ed | min-wil | Wat yu min? → What do you mean? |

### Mental and Cognitive

| Word | AFI | English | Present | Past | Future | Example |
|------|-----|---------|---------|------|--------|---------|
| **nof** | /nof/ | to know (facts) | nof-a | nof-ed | nof-wil | Mi nof wat 2+2 es. → I know what 2+2 is. |
| **bilever** | /biliver/ | to believe | biliver-a | biliver-ed | biliver-wil | Mi biliver in yu. → I believe in you. |
| **think** | /think/ | to think | think-a | think-ed | think-wil | Mi think so. → I think so. |
| **remembr** | /remembr/ | to remember | remembr-a | remembr-ed | remembr-wil | Mi remembr wat yu tal. → I remember what you said. |
| **forget** | /forget/ | to forget | forget-a | forget-ed | forget-wil | Mi forget evriwei. → I forget everywhere. |
| **undrstand** | /undredstand/ | to understand | undrstand-a | undrstand-ed | undrstand-wil | Mi undrstand VELA. → I understand VELA. |
| **memoriz** | /memoriz/ | to memorize | memoriz-a | memoriz-ed | memoriz-wil | Mi memoriz la wod-s. → I memorize the words. |
| **imajin** | /imajin/ | to imagine | imajin-a | imajin-ed | imajin-wil | Mi imajin la siti. → I imagine the city. |
| **desid** | /desid/ | to decide | desid-a | desid-ed | desid-wil | Mi desid tu kom. → I decide to come. |
| **sugest** | /sugest/ | to suggest | sugest-a | sugest-ed | sugest-wil | Mi sugest wat yu don. → I suggest what you do. |
| **explen** | /eksplen/ | to explain | explen-a | explen-ed | explen-wil | Mi explen la rule. → I explain the rule. |

### Possession and Exchange

| Word | AFI | English | Present | Past | Future | Example |
|------|-----|---------|---------|------|--------|---------|
| **hav** | /hav/ | to have | hav-a | hav-ed | hav-wil | Mi hav un kar. → I have a car. |
| **giv** | /giv/ | to give | giv-a | giv-ed | giv-wil | Mi giv yu la buk. → I give you the book. |
| **tek** | /tek/ | to take | tek-a | tek-ed | tek-wil | Yu tek la buk from mi. → You take the book from me. |
| **send** | /send/ | to send | send-a | send-ed | send-wil | Mi send la letr. → I send the letter. |
| **resiv** | /resiv/ | to receive | resiv-a | resiv-ed | resiv-wil | Mi resiv la paket. → I receive the package. |
| **share** | /sher/ | to share | share-a | share-ed | share-wil | Wi share la food. → We share the food. |
| **trid** | /trad/ | to trade | trad-a | trad-ed | trad-wil | De trad wit es otfr. → They trade with each other. |
| **sel** | /sel/ | to sell | sel-a | sel-ed | sel-wil | Mi sel la kar. → I sell the car. |
| **bai** | /bai/ | to buy | bai-a | bai-ed | bai-wil | Mi bai un hous. → I buy a house. |
| **pe** | /pe/ | to pay | pe-a | pe-ed | pe-wil | Mi pe for la diner. → I pay for the dinner. |
| **kost** | /kost/ | to cost | kost-a | kost-ed | kost-wil | La hous kost milyon. → The house costs a million. |
| **lent** | /lent/ | to lend | lent-a | lent-ed | lent-wil | Mi lent yu moni. → I lend you money. |
| **boro** | /boro/ | to borrow | boro-a | boro-ed | boro-wil | Mi boro moni from mi-fren. → I borrow money from my friend. |

### Social and Abstract Actions

| Word | AFI | English | Present | Past | Future | Example |
|------|-----|---------|---------|------|--------|---------|
| **help** | /help/ | to help | help-a | help-ed | help-wil | Mi help yu. → I help you. |
| **serv** | /serv/ | to serve | serv-a | serv-ed | serv-wil | Li serv la kofi. → She serves the coffee. |
| **joi** | /joi/ | to join | joi-a | joi-ed | joi-wil | Mi joi la klas. → I join the class. |
| **laf** | /laf/ | to laugh | laf-a | laf-ed | laf-wil | Wi laf at la stor. → We laugh at the story. |
| **kai** | /kai/ | to cry | kai-a | kai-ed | kai-wil | Baby kai. → Baby cries. |
| **sing** | /sing/ | to sing | sing-a | sing-ed | sing-wil | Mi sing la sonj. → I sing the song. |
| **dans** | /dans/ | to dance | dans-a | dans-ed | dans-wil | Wi dans evri noite. → We dance every night. |
| **ple** | /ple/ | to play | ple-a | ple-ed | ple-wil | La child ple in la gardn. → The child plays in the garden. |
| **rit** | /rit/ | to write | rit-a | rit-ed | rit-wil | Mi rit la letr. → I write the letter. |
| **rid** | /rid/ | to read | rid-a | rid-ed | rid-wil | Mi rid la buk. → I read the book. |
| **draw** | /draw/ | to draw | draw-a | draw-ed | draw-wil | La child draw la man. → The child draws the man. |
| **paint** | /peint/ | to paint | paint-a | paint-ed | paint-wil | Mi paint la wall. → I paint the wall. |
| **meis** | /meis/ | to measure | meis-a | meis-ed | meis-wil | Mi meis la rum. → I measure the room. |
| **kount** | /kount/ | to count | kount-a | kount-ed | kount-wil | Mi kount tu ten. → I count to ten. |
| **wait** | /weit/ | to wait | wait-a | wait-ed | wait-wil | Mi wait for yu. → I wait for you. |
| **kwik** | /kwik/ | to hurry | kwik-a | kwik-ed | kwik-wil | Mi kwik! | Hurry up! |
| **stop** | /stop/ | to stop | stop-a | stop-ed | stop-wil | Wi stop hier. → We stop here. |
| **start** | /start/ | to start | start-a | start-ed | start-wil | Wi start nau. → We start now. |
| **kontinyu** | /kontinyu/ | to continue | kontinyu-a | kontinyu-ed | kontinyu-wil | Wi kontinyu wok. → We continue working. |
| **chek** | /tcek/ | to check | chek-a | chek-ed | chek-wil | Mi chek la risolt. → I check the result. |

### Physical States and Change

| Word | AFI | English | Present | Past | Future | Example |
|------|-----|---------|---------|------|--------|---------|
| **grot** | /grot/ | to grow | grot-a | grot-ed | grot-wil | La plant grot. → The plant grows. |
| **die** | /dai/ | to die | die-a | die-ed | die-wil | La man die. → The man dies. |
| **born** | /born/ | to be born | born-a | born-ed | born-wil | Mi born in 1990. → I was born in 1990. |
| **heal** | /hial/ | to heal | heal-a | heal-ed | heal-wil | La wound heal. → The wound heals. |
| **slip** | /slip/ | to sleep | slip-a | slip-ed | slip-wil | Baby slip. → Baby sleeps. |
| **wek** | /wek/ | to wake up | wek-a | wek-ed | wek-wil | Mi wek at six. → I wake at six. |
| **rest** | /rest/ | to rest | rest-a | rest-ed | rest-wil | Mi rest nau. → I rest now. |
| **eat** | /it/ | to eat | eat-a | eat-ed | eat-wil | Mi eat laappl. → I eat the apple. |
| **drink** | /drink/ | to drink | drink-a | drink-ed | drink-wil | Mi drink watre. → I drink water. |
| **brit** | /brit/ | to breathe | brit-a | brit-ed | brit-wil | Mi brit evri dei. → I breathe every day. |
| **swet** | /swet/ | to sweat | swet-a | swet-ed | swet-wil | Mi swet aft la wok. → I sweat after the work. |
| **shin** | /sin/ | to shine | shin-a | shin-ed | shin-wil | La son shin. → The sun shines. |
| **rain** | /rein/ | to rain | rain-a | rain-ed | rain-wil | La rein fal. → The rain falls. |
| **snow** | /sno/ | to snow | snow-a | snow-ed | snow-wil | La snow fal. → The snow falls. |

### Modal Verbs

| Word | AFI | English | Structure | Example |
|------|-----|---------|-----------|---------|
| **kan** | /kan/ | can | kan + bare root | Mi kan si. → I can see. |
| **kan-ed** | /kan.ed/ | could | kan-ed + bare root | Mi kan-ed kom. → I could come. |
| **mas** | /mas/ | must | mas + bare root | Wi mas wok. → We must work. |
| **mas no** | /mas no/ | must not | mas no + bare root | De mas no kom. → They must not come. |
| **wan** | /wan/ | to want to | wan + bare root | Li wan kom. → She wants to come. |
| **shod** | /sod/ | should | shod + bare root | Yu shod kom. → You should come. |
| **ma** | /ma/ | may | ma + bare root | Mi ma kom. → I may come. |
| **wil** | /wil/ | will | wil + bare root | Mi wil kom. → I will come. |

---

## Section 3: Adjectives — **-im** suffix

*Adjectives come before the noun. Comparison: mor + im = comparative, mos + im = superlative.*

### Size and Physical Properties

| Word | AFI | English | Example | English |
|------|-----|---------|---------|---------|
| **big-im** | /big.im/ | big | un big-im hous | a big house |
| **smol** | /smol/ | small | un smol-im siti | a small city |
| **long-im** | /lon.im/ | long | un long-im rod | a long road |
| **short-im** | /sot.im/ | short | un short-im taim | a short time |
| **tall-im** | /tol.im/ | tall | un tall-im man | a tall man |
| **wid-im** | /wid.im/ | wide | la wid-im strit | the wide street |
| **narow** | /naro/ | narrow | la narow rod | the narrow road |
| **deep-im** | /dip.im/ | deep | la deep-im wel | the deep well |
| **shalow** | /salow/ | shallow | la shalow rivr | the shallow river |
| **thick-im** | /tik.im/ | thick | un thick-im buk | a thick book |
| **thin-im** | /tin.im/ | thin | un thin-im paper | a thin paper |
| **hevi-im** | /hevi.im/ | heavy | la hevi-im box | the heavy box |
| **lait-im** | /lait.im/ | light (weight) | la laimt box | the light box |
| **fast-im** | /fast.im/ | fast | la fast-im kar | the fast car |
| **slo** | /slo/ | slow | la slo-man | the slow man |

### Quality and Value

| Word | AFI | English | Example | English |
|------|-----|---------|---------|---------|
| **gud-im** | /gud.im/ | good | la gud-im man | the good man |
| **bad-im** | /bad.im/ | bad | la bad-im noz | the bad news |
| **new-im** | /nu.im/ | new | un new-im kar | a new car |
| **old-im** | /old.im/ | old | la old-im hous | the old house |
| **yong-im** | /jon.im/ | young | un yong-im gerl | a young girl |
| **nais-im** | /nais.im/ | nice / beautiful | la nais-im sonj | the nice song |
| **hapi-im** | /hapi.im/ | happy | la hapi-im famili | the happy family |
| **sad-im** | /sad.im/ | sad | Mi es sad. | I am sad. |
| **strong-im** | /stron.im/ | strong | la strong-im man | the strong man |
| **wik-im** | /wik.im/ | weak | la wik-im child | the weak child |
| **rich-im** | /ritc.im/ | rich | un rich-im man | a rich man |
| **poor-im** | /pur.im/ | poor | la poor-im famili | the poor family |
| **bizi-im** | /bizi.im/ | busy | Mi es bizi. | I am busy. |
| **fri-im** | /frai.im/ | free / free | Mi es fri. | I am free. |
| **safi-im** | /safi.im/ | safe | la safi-im hous | the safe house |
| **dangerus** | /dainjerus/ | dangerous | la danger
| **safi-im** | /safi.im/ | safe | la safi-im hous | the safe house |
| **dangerus** | /dainjerus/ | dangerous | la dangerus rod | the dangerous road |
| **clean-im** | /klian.im/ | clean | la clean-im rum | the clean room |
| **diti-im** | /diti.im/ | dirty | la diti-im masin | the dirty machine |
| **hardi-im** | /hardi.im/ | hard (material) | la hardi-im metal | the hard metal |
| **soft-im** | /soft.im/ | soft | la soft-im kusion | the soft cushion |
| **smooth-im** | /smud.im/ | smooth | la smooth-im surfes | the smooth surface |
| **ruf-im** | /ruf.im/ | rough | la ruf-im wol | the rough wool |
| **wet-im** | /wet.im/ | wet | la wet-im grund | the wet ground |
| **drai-im** | /drai.im/ | dry | la drai-im lond | the dry land |
| **warm-im** | /warm.im/ | warm | la warm-im son | the warm sun |
| **hot-im** | /hot.im/ | hot | la hot-im sup | the hot soup |
| **kol-im** | /kol.im/ | cold | la kol-im win | the cold wind |
| **kwikly** | /kwikli/ | quick | la kwikli man | the quick man |
| **sloli** | /sloli/ | slow | la sloli man | the slow man |
| **louud** | /loud/ | loud | la loud-im nois | the loud noise |
| **kwaiat** | /kwaiat/ | quiet | la kwaiat siti | the quiet city |
| **bright** | /brait/ | bright | la bright-im son | the bright sun |
| **dark-im** | /dark.im/ | dark | la dark-im noite | the dark night |
| **simpli-im** | /simpli.im/ | simple | la simpli-im sistem | the simple system |
| **komplex** | /kompleks/ | complex | la komplex sistem | the complex system |
| **eas-im** | /iz.im/ | easy | la eas-im wok | the easy work |
| **hard-im** | /hard.im/ | hard (difficult) | la hard-im kwestion | the hard question |
| **importent** | /importent/ | important | la importent man | the important man |
| **useful** | /usful/ | useful | la useful tool | the useful tool |
| **useles** | /usles/ | useless | la useles ting | the useless thing |
| **interesting** | /interisting/ | interesting | la interesting buk | the interesting book |
| **boring** | /boring/ | boring | la boring class | the boring class |
| **eksiting** | /eksaiting/ | exciting | la eksiting filem | the exciting film |
| **expensive** | /ekspensiv/ | expensive | la expensive kar | the expensive car |
| **cheap-im** | /tcip.im/ | cheap | la cheap-im product | the cheap product |
| **popular** | /popiular/ | popular | la popular artst | the popular artist |
| **famos** | /famos/ | famous | la famoso skul | the famous school |
| **sekri** | /sekri/ | secret | la sekri plans | the secret plans |
| **public** | /publik/ | public | la public parks | the public parks |
| **privat** | /privat/ | private | la privat lif | the private life |
| **natural** | /natural/ | natural | la natural rsoers | the natural resources |
| **artficial** | /artficial/ | artificial | la artficial intelijens | the artificial intelligence |

### Color Adjectives

| Word | AFI | English | Example |
|------|-----|---------|---------|
| **blak** | /blak/ | black | la blak kat |
| **wait** | /wait/ | white | la wait snow |
| **red-im** | /red.im/ | red | la red-im rozs |
| **blu** | /blu/ | blue | la blu skai |
| **gren** | /gren/ | green | la gren tri |
| **yelo** | /jelo/ | yellow | la yelo sun |
| **orange** | /oranj/ | orange | la orange frukt |
| **purpl** | /purpl/ | purple | la purpl flauor |
| **grin** | /grin/ | gray | la grin wal |
| **broun** | /braun/ | brown | la broun eirt |
| **gol-im** | /gol.im/ | golden | la gol-im sun |
| **silv-im** | /silv.im/ | silver | la silv-im moon |

### Emotional and Mental States

| Word | AFI | English | Example |
|------|-----|---------|---------|
| **hapi** | /hapi/ | happy | Mi es hapi. |
| **sad** | /sad/ | sad | La child es sad. |
| **angri** | /angri/ | angry | La man es angri. |
| **afred** | /afred/ | afraid | Mi es afred. |
| **traid** | /trait/ | afraid/scared | |
| **surprisd** | /surprisd/ | surprised | |
| **confyuzd** | /konfaiuzd/ | confused | |
| **kongident** | /kongident/ | confident | |
| **shy** | /sai/ | shy | |
| **lonli** | /lonli/ | lonely | |
| **tired** | /taird/ | tired | Mi es tri. |
| **tri** | /tri/ | tired (short) | Mi es tri. |
| **hagri** | /hagri/ | hungry | Mi es hagri. |
| **torsti** | /torsti/ | thirsty | Mi es torsti. |
| **sik-im** | /sik.im/ | sick | La man es sik. |
| **helthi** | /helthi/ | healthy | La famili es helthi. |
| **slepi** | /slepi/ | sleepy | Mi es slepi. |
| **bored** | /bord/ | bored | Mi es bored. |
| **proud** | /praud/ | proud | Li es proud. |
| **grett** | /gret/ | great | La idea es gret! |
| **wonderful** | /wanderful/ | wonderful | La dei es wonderful! |

### Comparison Forms

| Positive | Comparative (mor + im) | Superlative (mos + im) |
|----------|----------------------|----------------------|
| big-im | mor big-im | mos big-im |
| gud-im | mor gud-im | mos gud-im |
| smol | mor smol | mos smol |
| hapi | mor hapi | mos hapi |
| strong-im | mor strong-im | mos strong-im |
| fast-im | mor fast-im | mos fast-im |

---

## Section 4: Nouns — Organized by Semantic Domain

### The Human Body

| Word | AFI | English | Compound notes | Example |
|------|-----|---------|---------------|---------|
| **bodi** | /bodi/ | body | | Mi-se bodi es strong. |
| **head** | /hed/ | head | | La head es top of bodi. |
| **fut** | /fut/ | foot/feet | | La man wok on fut. |
| **hand** | /hand/ | hand | | Mi-se hand es big. |
| **fingar** | /fingar/ | finger | | Wan hand hav five fingars. |
| **leg** | /leg/ | leg | | La leg es strong. |
| **arm** | /arm/ | arm | | La arm es strong. |
| **bak** | /bak/ | back | | Mi-se bak es sik. |
| **fas** | /fas/ | face | | La fas es nais. |
| **ai** | /ai/ | eye | | Mi hai tu ais. |
| **nos** | /nos/ | nose | | Mi nos es big. |
| **mous** | /maus/ | mouth | | La mous es smol. |
| **ear** | /ir/ | ear | | La ear her la sound. |
| **hair** | /her/ | hair | | La hair es long. |
| **skin** | /skin/ | skin | | La skin es soft. |
| **blud** | /blud/ | blood | | La blud flo in bodi. |
| **bon** | /bon/ | bone | | La bon es hardi. |
| **miscl** | /miskl/ | muscle | | La miscl es strong. |
| **brain** | /brein/ | brain | | La brain think. |
| **hert** | /hert/ | heart | | La hert bit. |
| **long** | /lon/ | lung | | La long mak breath. |
| **stomok** | /stomok/ | stomach | | Mi-se stomok es hagri. |
| **livr** | /livr/ | liver | | | 
| **kidni** | /kidni/ | kidney | | 
| **tung** | /tun/ | tongue | | La tung tas. |
| **te** | /te/ | tooth/teeth | | Mi hav thirty-te. |
| **berd** | /berd/ | beard | | La man hav berd. |
| **babi-fas** | /babi.fas/ | cheek | baby-face | |

### Family and Relationships

| Word | AFI | English | Compound notes | Example |
|------|-----|---------|---------------|---------|
| **famili** | /famili/ | family | ← Latin, recogn. all | La famili es big. |
| **man** | /man/ | man / person | | Un man kom. |
| **wuman** | /wuman/ | woman | | La wuman es nais. |
| **child** | /tcild/ | child | | Un child ple. |
| **gerl** | /gerl/ | girl | | Un gerl sing. |
| **boy** | /boi/ | boy | | Un boy luk. |
| **baby** | /beibi/ | baby | | Un baby slip. |
| **fren** | /fren/ | friend | | Mi-se fren es hapi. |
| **paran** | /paran/ | parent | | Mi-se paran es old. |
| **fatr** | /fatr/ | father | | Mi-se fatr es strong. |
| **motr** | /motr/ | mother | | Mi-se motr es kwin. |
| **son** | /son/ | son | | La son wok. |
| **dafter** | /dofter/ | daughter | | La dafter lern. |
| **brotr** | /brotr/ | brother | | Mi-se brotr es tall. |
| **sisr** | /sistr/ | sister | | Mi-se sisr es smol. |
| **granparan** | /granparan/ | grandparent | | La granparan es old. |
| **granfatr** | /granfatr/ | grandfather | | Mi-se granfatr liv hier. |
| **granmotr** | /granmotr/ | grandmother | | La granmotr kuk wel. |
| **unkl** | /unkl/ | uncle | | 
| **aunt** | /ant/ | aunt | | 
| **kuzn** | /kuzn/ | cousin | | 
| **nifaw** | /nifaw/ | nephew/niece | | 
| **wif** | /waf/ | wife | | Mi-se wif es nais. |
| **hasband** | /hasband/ | husband | | Mi-se hasband wok. |
| **neibr** | /neibr/ | neighbor | | Mi-se neibr es fri. |
| **ppl** | /pi.pi.el/ | people | pp-le = many persons | La ppl es big. |
| **man-s** | /man.s/ | men / people | plural | De man-s wok. |
| **wuman-s** | /wuman.s/ | women | plural | Dese wuman-s es nais. |
| **child-s** | /tcild.s/ | children | plural | Dese child-s ple. |

### Society and Roles

| Word | AFI | English | Compound notes | Example |
|------|-----|---------|---------------|---------|
| **popul** | /popiul/ | people / population | | La popul es big. |
| **siti** | /siti/ | city | ← Latin (urbs), recogn. | Wi liv la siti-te. |
| **vilaj** | /vilaj/ | village | | La vilaj es smol. |
| **naishon** | /naishon/ | nation | | La naishon es big. |
| **kantri** | /kantri/ | country | | La kantri es beautiful. |
| **gvrnmnt** | /gvnmnt/ | government | | La gvrnmnt mak law-s. |
| **law** | /la/ | law | | La law es importent. |
| **soldier** | /soldier/ | soldier | | 
| **polis** | /polis/ | police | | La polis kom. |
| **polis-man** | /polis.man/ | policeman | | La polis-man wok. |
| **doktr** | /doktr/ | doctor | | Mi go tu doktr. |
| **nurs** | /nurs/ | nurse | | La nurs es kind. |
| **tice** | /tiatc/ | teacher | | La tice lern child-s. |
| **student** | /student/ | student | | Un student stodi. |
| **droctor** | /droctor/ | doctor | | Mi es droctor. |
| **lavyr** | /lavier/ | lawyer | | La lafyer werk in la curt. |
| **injinir** | /injinir/ | engineer | | La injinir build bridj. |
| **artis** | /artis/ | artist | | La artis paint la pictiur. |
| **muzishn** | /muzishn/ | musician | | La muzishn sing. |
| **ritr** | /ritr/ | writer | | La ritr rit la buk. |
| **bos** | /bos/ | boss | | Mi-se bos es bizi. |
| **workr** | /workr/ | worker | | La workr wok hard. |
| **biznisman** | /biznisman/ | businessman | | 
| **famer** | /famer/ | farmer | | La famer grow crop-s. |
| **fisher** | /fisher/ | fisherman | | La fisher kom from la se. |
| **kuk** | /kuk/ | cook | | La kuk mak gud food. |
| **driver** | /driver/ | driver | | La driver stop la kar. |
| **soldier** | /soldier/ | soldier | | La soldier protec la kantri. |
| **prezidnt** | /prezidnt/ | president | | La prezidnt liv in la palasyo. |
| **king** | /king/ | king | | La king es rich. |
| **kwain** | /kwain/ | queen | | La kwain es beautiful. |
| **citizn** | /sitizn/ | citizen | | Mi es sitizn of la naishon. |

### The Home and Buildings

| Word | AFI | English | Compound notes | Example |
|------|-----|---------|---------------|---------|
| **hous** | /haus/ | house | | La hous es big. |
| **houskel** | /haus.kel/ | household | hous + kel (people of house) | La houskel hav five man-s. |
| **hom** | /hom/ | home | | Mi go hom. |
| **rum** | /rum/ | room | | La rum es big. |
| **bedrum** | /bedrum/ | bedroom | bed + rum | Mi slip in la bedrum. |
| **kitcen** | /kitcen/ | kitchen | | Mi kuk in la kitcen. |
| **baterom** | /baterom/ | bathroom | | Mi wosh in la baterom. |
| **livrum** | /livrum/ | living room | | Wi tok in la livrum. |
| **gardn** | /gardn/ | garden | | La child ple in la gardn. |
| **dor** | /dor/ | door | | Mi opn la dor. |
| **windo** | /windo/ | window | | La windo es opn. |
| **wal** | /wal/ | wall | | La wal es wit. |
| **flor** | /flor/ | floor | | La flor es klean. |
| **seiling** | /seiling/ | ceiling | | La seiling es high. |
| **rof** | /rof/ | roof | | La rof es new. |
| **stair** | /stair/ | stairs | | Mi gol up la stair. |
| **chimni** | /cimni/ | chimney | | La smok kom from la chimni. |
| **tebul** | /tebul/ | table | | La buk es on la tebul. |
| **ceir** | /seir/ | chair | | Mi sit on la ceir. |
| **sof** | /sof/ | sofa | | Wi liv on la sof. |
| **bed** | /bed/ | bed | | Baby slip in la bed. |
| **lamp** | /lamp/ | lamp | | La lamp shin. |
| **telivizion** | /telivizion/ | television | | Mi si la telivizion. |
| **swic** | /switc/ | switch | | Mi push la swic. |
| **lok** | /lok/ | lock | | Mi lok la dor. |
| **key** | /ki/ | key | | Mi hav la key. |
| **bilding** | /bilding/ | building | | La bilding es tall. |
| **skol** | /skol/ | school | | Child-s go tu skol. |
| **ofis** | /ofis/ | office | | Mi wok in la ofis. |
| **hospital** | /hospital/ | hospital | | Mi go tu hospital. |
| **hotl** | /hotl/ | hotel | | Wi liv in la hotl. |
| **shurch** | /stcrc/ | church | | Wi go tu la shurch. |
| **tempol** | /tempol/ | temple | | 
| **mosk** | /mosk/ | mosque | | 
| **park** | /park/ | park | | Wi wok in la park. |
| **strit** | /strit/ | street | | La strit es wide. |
| **rod** | /rod/ | road | | La rod es long. |
| **wai** | /wai/ | way / road | | Weit! Dis es la rong wai. |
| **bridj** | /brij/ | bridge | | Wi kros la bridj. |
| **tunnel** | /tunnel/ | tunnel | | Wi go tru la tunnel. |

### Nature and Environment

| Word | AFI | English | Compound notes | Example |
|------|-----|---------|---------------|---------|
| **son** | /son/ | sun | ← Latin: sol = all recognize | La son shin. |
| **moon** | /mun/ | moon | | La moon es round. |
| **star** | /star/ | star | | La star-s shine at noite. |
| **skai** | /skai/ | sky | | La skai es blu. |
| **watre** | /watre/ | water | ← international | Mi drink watre. |
| **fai** | /fai/ | fire | | La fai burin. |
| **erd** | /erd/ | earth | | La erd es round. |
| **lond** | /lond/ | land | | La lond es big. |
| **se** | /se/ | sea / ocean | | Wi swim in la se. |
| **rivr** | /rivr/ | river | | La rivr flo. |
| **lek** | /lek/ | lake | | Wi swim in la lek. |
| **mautin** | /mautin/ | mountain | | La mautin es high. |
| **hil** | /hil/ | hill | | Wi climb la hil. |
| **fores** | /fores/ | forest | | La fores es big. |
| **tri** | /tri/ | tree | | La tri es green. |
| **gras** | /gras/ | grass | | La gras es green. |
| **flauor** | /flauor/ | flower | | La flauor es nais. |
| **plant** | /plant/ | plant | | Mi grow plant-s. |
| **wud** | /wud/ | wood | | La tri es mad from wud. |
| **sand** | /sand/ | sand | | La sand es yelo. |
| **ston** | /ston/ | stone | | La ston es hardi. |
| **waild** | /waild/ | world | ← Latin: mundus, recogn. | La waild es big. |
| **kantin** | /kantin/ | continent | | 
| **iland** | /iland/ | island | | 
| **bech** | /bitc/ | beach | | Wi relaks on la bech. |
| **wo** | /wo/ | wave | | La wo kom and go. |
| **rein** | /rein/ | rain | | La rein fal. |
| **snow** | /sno/ | snow | | La snow fal in winter. |
| **wind** | /wind/ | wind | | La wind blow. |
| **kraud** | /kraud/ | cloud | | La kraud-s es wit. |
| **sarkis** | /sarkis/ | thunder | | La sarkis kom aft la rein. |
| **letning** | /letning/ | lightning | | La letning shain. |
| **storm** | /storm/ | storm | | La storm es dangerus. |
| **tempratcur** | /tempratcur/ | temperature | | La tempratcur es hot. |
| **hot** | /hot/ | heat | | La hot es strong in somr. |
| **kold** | /kold/ | cold | | La kold es strong in winter. |
| **climat** | /klimait/ | climate | | La klimait es warm. |
| **envirnmnt** | /envirnmnt/ | environment | | La envirnmnt es importent. |
| **naitur** | /naitur/ | nature | | Naitur es beautiful. |
| **animal** | /animal/ | animal | | 
| **bird** | /bird/ | bird | | La bird sing. |
| **fish** | /fis/ | fish | | Wi katch fish. |
| **dog** | /dog/ | dog | | Mi dog es big. |
| **kat** | /kat/ | cat | | La kat sleep. |
| **hors** | /hors/ | horse | | Wi rid la hors. |
| **cau** | /kau/ | cow | | La cau giv milk. |
| **shep** | /sep/ | sheep | | La shep es soft. |
| **pig** | /pig/ | pig | | 
| **baet** | /baet/ | bat | | 
| **snak** | /snak/ | snake | | La snak es dangerus. |
| **insect** | /insect/ | insect | | 
| **buterflai** | /buterflai/ | butterfly | butterfly | 

### Food and Drink

| Word | AFI | English | Compound notes | Example |
|------|-----|---------|---------------|---------|
| **food** | /fud/ | food | | La food es gud. |
| **appl** | /apel/ | apple | | Mi eat un appl. |
| **banan** | /banan/ | banana | | 
| **oranj** | /oranj/ | orange | | 
| **grap** | /grap/ | grape | | 
| **beri** | /beri/ | berry | | 
| **strobri** | /strobri/ | strawberry | | 
| **lemon** | /lemon/ | lemon | | 
| **mang** | /mang/ | mango | | 
| **watrmeln** | /watrmeln/ | watermelon | | 
| **frukt** | /frunkt/ | fruit | | Frukt es nais and helthi. |
| **vegi** | /veji/ | vegetable | | 
| **korn** | /korn/ | corn | | 
| **ris** | /ris/ | rice | | 
| **bred** | /bred/ | bread | | Mi eat bred. |
| **mitn** | /mitn/ | meat | | Mi eat mitn. |
| **sisn** | /sisen/ | chicken | | 
| **bef** | /bef/ | beef | | 
| **pork** | /pork/ | pork | | 
| **fis** | /fis/ | fish | | 
| **eg** | /eg/ | egg | | Mi fried two eg-s. |
| **milk** | /milk/ | milk | | La baby drink milk. |
| **ces** | /tes/ | cheese | | 
| **butr** | /butr/ | butter | | 
| **cream** | /kriam/ | cream | | 
| **sup** | /sup/ | soup | | La sup es hot. |
| **salad** | /salad/ | salad | | 
| **salt** | /solt/ | salt | | 
| **sugr** | /sukr/ | sugar | | 
| **oil** | /oil/ | oil | | 
| **vinegr** | /vinegr/ | vinegar | | 
| **spice** | /spais/ | spice | | 
| **pepper** | /peper/ | pepper | | 
| **te** | /te/ | tea | | Mi drink te. |
| **kofi** | /kofi/ | coffee | | Mi drink kofi evri moring. |
| **juice** | /juis/ | juice | | 
| **soda** | /soda/ | soda | | 
| **win** | /win/ | wine | | 
| **birr** | /bir/ | beer | | 
| **watre** | /watre/ | water | | Mi drink watre. |
| **chokolat** | /tokolat/ | chocolate | | Mi love chokolat. |
| **keik** | /keik/ | cake | | La keik es swit. |
| **sop** | /sop/ | soap | | 

### Numbers

| Word | AFI | English | Word | AFI | English |
|------|-----|---------|------|-----|---------|
| **zero** | /zero/ | 0 | **ten** | /ten/ | 10 |
| **wan** | /wan/ | 1 | **elevn** | /elevn/ | 11 |
| **tu** | /tu/ | 2 | **twelv** | /twelv/ | 12 |
| **tri** | /tri/ | 3 | **ten-tri** | /ten.tri/ | 13 |
| **for** | /for/ | 4 | **fourtin** | /fortin/ | 14 |
| **faiv** | /faiv/ | 5 | **fiftin** | /fiftin/ | 15 |
| **siks** | /siks/ | 6 | **sixtin** | /sixtin/ | 16 |
| **sevn** | /sevn/ | 7 | **sevtin** | /sevtin/ | 17 |
| **eit** | /eit/ | 8 | **eittin** | /eittin/ | 18 |
| **nain** | /nain/ | 9 | **naintin** | /naintin/ | 19 |
| | | | **twenti** | /twenti/ | 20 |
| | | | **twenti-wan** | /twenti.wan/ | 21 |
| | | | **thirti** | /thirti/ | 30 |
| | | | **forti** | /forti/ | 40 |
| | | | **fifti** | /fifti/ | 50 |
| | | | **siksti** | /siksti/ | 60 |
| | | | **seventi** | /seventi/ | 70 |
| | | | **eiti** | /eiti/ | 80 |
| | | | **nainti** | /nainti/ | 90 |
| | | | **handrd** | /handrd/ | 100 |
| | | | **handrd-wan** | /handrd.wan/ | 101 |
| | | | **thausand** | /thausand/ | 1,000 |
| | | | **miliyun** | /miliyun/ | 1,000,000 |

### Time and Seasons

| Word | AFI | English | Example |
|------|-----|---------|---------|
| **yeer** | /jir/ | year | Wan yeer hav twelv munts. |
| **month** | /month/ | month | Wot month es nau? |
| **munts** | /munts/ | month | La munts of yeer. |
| **wik** | /wik/ | week | Wan wik hav seven dei-s. |
| **dei** | /dei/ | day | Nau-dei mi wok. |
| **nour** | /nauur/ | hour | Wan nour es sixty minimit-s. |
| **minimit** | /minimit/ | minute | 
| **sekond** | /sekond/ | second | 
| **moring** | /moring/ | morning | Mi wok evri moring. |
| **noon** | /nun/ | noon | La sun es at top at noon. |
| **aftnaun** | /aftnaun/ | afternoon | 
| **eveing** | /iveing/ | evening | 
| **noit** | /noit/ | night | La star-s shine at noit. |
| **midnaight** | /midnait/ | midnight | La midnaight es very kwaiat. |
| **spring** | /spring/ | spring | Plant-s gro in spring. |
| **samr** | /samr/ | summer | La sun es strong in samr. |
| **fol** | /fol/ | fall / autumn | La leaf-s fal in fol. |
| **winter** | /winter/ | winter | La snow fal in winter. |
| **centuri** | /senturi/ | century | Wan centuri es hundred yeer-s. |
| **agen** | /agen/ | ago | Tri yeer-s agen mi liv in la siti. |
| **bifor** | /bifor/ | before | Bide bifor la storm. |
| **aft** | /aft/ | after | Aft la wok, mi kom hom. |

### Communication and Media

| Word | AFI | English | Compound notes | Example |
|------|-----|---------|---------------|---------|
| **wod** | /wod/ | word | | Mi lern nu wod-s. |
| **langwij** | /langwii/ | language | | VELA es nu langwij. |
| **buk** | /buk/ | book | ← Latin: liber, recogn. | Mi rid la buk. |
| **paper** | /peiper/ | paper | | Mi rit on la paper. |
| **pen** | /pen/ | pen | | Mi rit wit pen. |
| **pensil** | /pensil/ | pencil | | 
| **ink** | /ink/ | ink | | 
| **postr** | /postr/ | poster | | 
| **notis** | /notis/ | notice | | 
| **sain** | /sain/ | sign | | La sain es big. |
| **map** | /map/ | map | | Mi luk at la map. |
| **piktur** | /piktur/ | picture | | La piktur es nais. |
| **foto** | /foto/ | photo | | Mi tak un foto. |
| **film** | /film/ | film / movie | | Wi si la film. |
| **sonj** | /sonj/ | song | | Wi sing la sonj. |
| **muzik** | /muzik/ | music | | Mi lik la muzik. |
| **riten** | /riten/ | writing | | 
| **stor** | /stor/ | story | | La stor es interesting. |
| **nius** | /nius/ | news | | La nius es sad. |
| **kanal** | /kanal/ | channel | | La kanal brodkast la program. |
| **radia** | /radia/ | radio | | Mi lisn tu la radia. |
| **teliviz** | /teliviz/ | television | | Mi si la telivizion. |
| **internet** | /internet/ | internet | | Wi komunikat via internet. |
| **meil** | /meil/ | mail | | Mi resiv meil. |
| **letr** | /letr/ | letter | | Mi rit un letr. |
| **paket** | /paket/ | package | | Mi resiv un paket. |
| **telefon** | /telefon/ | telephone | | Mi kal on la telefon. |
| **mesij** | /mesij/ | message | | Mi send un mesij. |
| **konversashon** | /konversashon/ | conversation | | La konversashon es interesting. |
| **newspaper** | /nuspeiper/ | newspaper | | Mi rid la newspaper evri moring. |
| **komiks** | /komiks/ | comics | | 
| **diksbri** | /diksbri/ | dictionary | | Mi luku in la diksbri. |
| **gramr** | /gramr/ | grammar | | VELA gramr es simpli. |
| **vokabulari** | /vokabulari/ | vocabulary | | Mi lern nu vokabulari. |

### Technology and Tools

| Word | AFI | English | Compound notes | Example |
|------|-----|---------|---------------|---------|
| **kompiutr** | /kompiutr/ | computer | | Mi wok on la kompiutr. |
| **laptap** | /laptap/ | laptop | | 
| **fon** | /fon/ | phone | | Mi her mi fon. |
| **mobil** | /mobil/ | mobile | | Mi fon es mobil. |
|


### Technology and Tools (continued)

| Word | AFI | English | Compound notes | Example |
|------|-----|---------|---------------|---------|
| **aplikaishon** | /aplikaishon/ | application / app | | Mi aplikaishon es useful. |
| **smaurtfon** | /smartfon/ | smartphone | | La smartfon es powerful. |
| **kamera** | /kamera/ | camera | | Mi tak foto wit kamera. |
| **speaker** | /spiker/ | speaker | | |
| **screen** | /skrin/ | screen | | La screen es big. |
| **buton** | /buton/ | button | | Mi push la buton. |
| **batrei** | /batrei/ | battery | | La batrei es low. |
| **charger** | /tcarger/ | charger | | Mi charch la fon. |
| **softwer** | /softwer/ | software | | La softwer es modern. |
| **hardwer** | /hardwer/ | hardware | | |
| **program** | /program/ | program | | La program es useful. |
| **webseit** | /webseit/ | website | | |
| **server** | /server/ | server | | |
| **data** | /data/ | data | | La data es importent. |
| **kloud** | /klaud/ | cloud | | La data es in la kloud. |
| **sityu** | /situ/ | city (modern) | | |
| **wifi** | /wifi/ | Wi-Fi | | La wifi es fast. |
| **paswrd** | /paswrd/ | password | | Mi Paswrd es sekri. |
| **akount** | /akaunt/ | account | | Mi akount es secure. |
| **profil** | /profil/ | profile | | |
| **nettwerk** | /netwerk/ | network | | |
| **virus** | /virus/ | virus | | La virus es dangerus. |
| **passport** | /pasport/ | passport | | |
| **bilet** | /bilet/ | ticket | | Mi bai un bilet. |
| **masin** | /masin/ | machine | | La masin wok wel. |
| **engin** | /engin/ | engine | | La engin of la kar. |
| **traktor** | /traktor/ | tractor | | |
| **robot** | /robot/ | robot | | La robot wok fast. |
| **tool** | /tul/ | tool | | Mi masin gav la tool-s. |
| **hammer** | /hammer/ | hammer | | |
| **nail** | /neil/ | nail | | |
| **skru** | /skru/ | screw | | |
| **knif** | /knaf/ | knife | | La knif es sharp. |
| **fork** | /fork/ | fork | | |
| **sponn** | /spon/ | spoon | | |
| **plat** | /plat/ | plate | | |
| **glas** | /glas/ | glass | | Mi drink watre from glas. |
| **kup** | /kup/ | cup | | Mi drink kofi from kup. |
| **botl** | /botl/ | bottle | | |
| **bag** | /bag/ | bag | | |
| **boks** | /boks/ | box | | |
| **kontener** | /kontener/ | container | | |
| **rope** | /rop/ | rope | | |
| **tuai** | /tuai/ | towel | | |
| **umbrel** | /umbrel/ | umbrella | | Mi hold la umbrel in la rein. |

### Transportation

| Word | AFI | English | Compound notes | Example |
|------|-----|---------|---------------|---------|
| **kar** | /kar/ | car | | La kar es fast. |
| **truk** | /truk/ | truck | | |
| **bis** | /bis/ | bus | | Wi go wit la bis. |
| **trein** | /trein/ | train | | Wi travel by trein. |
| **bikl** | /bikl/ | bicycle | | Mi rid la bikl. |
| **motorbaik** | /motorbaik/ | motorcycle | | |
| **aereplan** | /aereplan/ | airplane | | Wi flai by aereplan. |
| **ship** | /sip/ | ship | | Wi travl by ship. |
| **boot** | /but/ | boat | | |
| **taxi** | /taksi/ | taxi | | Mi teik la taxi. |
| **ambulans** | /ambulans/ | ambulance | | La ambulans kom fast. |
| **polis-kar** | /polis.kar/ | police car | | |
| **trein-stashon** | /trein.stashon/ | train station | | Wi asiv at la trein-stashon. |
| **aereport** | /aereport/ | airport | | |
| **port** | /port/ | port | | |
| **strit** | /strit/ | street | | |
| **motorway** | /motorwei/ | motorway | | |
| **gas** | /gas/ | gasoline | | Mi bai gas for la kar. |
| **petrol** | /petrol/ | petrol | | |
| **trafik** | /trafik/ | traffic | | La trafik es bizi. |
| **pasengr** | /pasengr/ | passenger | | |
| **driver** | /driver/ | driver | | La driver stop la kar. |
| **pilat** | /pilat/ | pilot | | La pilat kontrol la aereplan. |
| **sailr** | /seilr/ | sailor | | |

### Work and Business

| Word | AFI | English | Compound notes | Example |
|------|-----|---------|---------------|---------|
| **biznes** | /biznes/ | business | | Mi hav un biznes. |
| **wok** | /wok/ | work / job | | Mi wok evri dei. |
| **kompani** | /kompani/ | company | | La kompani es big. |
| **faktori** | /faktori/ | factory | | La faktori mak product-s. |
| **shop** | /sop/ | shop | | Mi bai food from shop. |
| **market** | /market/ | market | | La market es bizi. |
| **stor** | /stor/ | store | | |
| **restorant** | /restorant/ | restaurant | | Wi it in la restorant. |
| **bank** | /bank/ | bank | | Mi go tu bank. |
| **moni** | /moni/ | money | | Mi need mor moni. |
| **sallery** | /saleri/ | salary | | Mi resiv sallery evri month. |
| **prize** | /praiz/ | prize / price | | La prize es high. |
| **profil** | /profil/ | profit | | La biznes mak profil. |
| **kosl** | /kost/ | cost | | La kost es low. |
| **sel** | /sel/ | sale | | La sel es big. |
| **shopping** | /soping/ | shopping | | Mi du shopping evri wik. |
| **kontract** | /kontract/ | contract | | Mi sign la kontract. |
| **miting** | /miting/ | meeting | | Wi hav miting at three. |
| **klient** | /klient/ | client | | Mi klient es importent. |
| **konsumer** | /konsumer/ | consumer | | |
| **product** | /product/ | product | | La product es gud. |
| **brand** | /brand/ | brand | | |
| **advrtais** | /advrtais/ | advertisement | | |
| **kastomer** | /kastomer/ | customer | | La kastomer es importent. |
| **korporashon** | /korporashon/ | corporation | | |

### Government and Law

| Word | AFI | English | Compound notes | Example |
|------|-----|---------|---------------|---------|
| **konstitushon** | /konstitushon/ | constitution | | |
| **elekshon** | /elekshon/ | election | | La elekshon es in Novembr. |
| **vote** | /vot/ | vote | | Mi vote for la prezidnt. |
| **parti** | /parti/ | party (political) | | |
| **kontri** | /kontri/ | country | | Mi kontri es Venesuela. |
| **law** | /la/ | law | | La law protec sitizn-s. |
| **rule** | /rul/ | rule | | La rule es clar. |
| **kort** | /kort/ | court | | La lafyer go tu kort. |
| **juri** | /juri/ | jury | | La juri desid. |
| **juis** | /juis/ | judge | | La juis diksid la kas. |
| **kriminal** | /kriminal/ | criminal | | La kriminal es kold. |
| **krime** | /kraim/ | crime | | La krime es bad. |
| **prison** | /prison/ | prison | | La kriminal go tu prison. |
| **tax** | /taks/ | tax | | Mi pe tax evri yeer. |
| **army** | /armi/ | army | | |
| **navy** | /nevi/ | navy | | |
| **afyr** | /afyr/ | air force | | |
| **rifuj** | /rifuj/ | refuge | | |
| **amnbasy** | /ambasy/ | embassy | | |

### Science and Health

| Word | AFI | English | Compound notes | Example |
|------|-----|---------|---------------|---------|
| **syens** | /saiens/ | science | | Syens es importent. |
| **sientist** | /saientist/ | scientist | | La sientist reserch. |
| **medisin** | /medisin/ | medicine / drug | | La medisin kure la siknes. |
| **siknes** | /siknes/ | sickness / disease | | La siknes es bad. |
| **healt** | /helt/ | health | | Healt es welth. |
| **bodifikashon** | /bodifikashon/ | body modification | | |
| **injuri** | /injuri/ | injury | | La injuri es serius. |
| **akcident** | /aksident/ | accident | | La akcident es sad. |
| **kur** | /kur/ | cure | | La medisin kur la siknes. |
| **privent** | /privent/ | prevention | | |
| **bitamin** | /bitamin/ | vitamin | | |
| **vacine** | /vasin/ | vaccine | | |
| **alerji** | /alerji/ | allergy | | |
| **pain** | /pein/ | pain | | La pain es strong. |
| **wond** | /wond/ | wound | | La wond heal. |
| **bruis** | /bruis/ | bruise | | |
| **fraktur** | /fraktur/ | fracture | | |
| **sergeri** | /sergeri/ | surgery | | |
| **doktr** | /doktor/ | doctor | | Mi go tu doktr. |
| **nurs** | /nurs/ | nurse | | |
| **aptikeri** | /aptikeri/ | pharmacy | | Mi go tu aptikeri. |
| **medikl** | /medikl/ | medical | | |
| **bioloji** | /bioloji/ | biology | | |
| **kimistri** | /kimistri/ | chemistry | | |
| **fiziks** | /fiziks/ | physics | | |
| **matamatiks** | /matamatiks/ | mathematics | | Mi lern matamatiks. |
| **nombr** | /nombr/ | number | | Wan nomber es twenty. |
| **formul** | /formul/ | formula | | |
| **eksperimnt** | /eksperimnt/ | experiment | | La sientist don eksperimnt. |
| **theory** | /teori/ | theory | | |
| **diskovr** | /diskovr/ | discovery | | La diskovr es importent. |
| **technoloji** | /teknoloji/ | technology | | Technoloji es modern. |
| **invenshon** | /invenshon/ | invention | | La invenshon change la waild. |
| **intilijens** | /intelijens/ | intelligence | | Artficial intelijens es powerful. |
| **robotiks** | /robotiks/ | robotics | | |

### Abstract and Emotional Concepts

| Word | AFI | English | Compound notes | Example |
|------|-----|---------|---------------|---------|
| **idea** | /aidea/ | idea | | La idea es simpli. |
| **plan** | /plan/ | plan | | Mi mak un plan. |
| **sistem** | /sistem/ | system | | La sistem es komplex. |
| **problem** | /problem/ | problem | | La problem es big. |
| **solushon** | /solushon/ | solution | | La solushon es esi. |
| **risolt** | /rizolt/ | result | | La risolt es gud. |
| **reason** | /rizon/ | reason | | Wat es la reason? |
| **efect** | /efect/ | effect | | La efect es big. |
| **caus** | /kauz/ | cause | | La caus es unkown. |
| **chans** | /tcans/ | chance | | La chans es smol. |
| **risk** | /risk/ | risk | | La risk es big. |
| **sukses** | /sukses/ | success | | Sukses es gud. |
| **faiolr** | /faiolr/ | failure | | Faiolr es part of lerning. |
| **progress** | /progress/ | progress | | Wi mak progress. |
| **chang** | /tcainj/ | change | | La chang es rapid. |
| **dificlti** | /dificlti/ | difficulty | | La dificlti es big. |
| **oprtuniti** | /oportiuniti/ | opportunity | | La oprtuniti es big. |
| **abilliti** | /abiliti/ | ability | | Mi abilliti es big. |
| **power** | /pauer/ | power | | La power es strong. |
| **respekt** | /respekt/ | respect | | Mi respet yu. |
| **respons** | /respons/ | responsibility | | La respons es big. |
| **right** | /rait/ | right (correct) | | Yu es rait. |
| **rong** | /rong/ | wrong | | La anser es rong. |
| **tru** | /tru/ | true | | La stori es tru. |
| **fols** | /fols/ | false | | La klaim es fols. |
| **importent** | /importent/ | important | | La wok es importent. |
| **normal** | /normal/ | normal | | La sitiuashon es normal. |
| **speshal** | /speshal/ | special | | La dei es speshal. |
| **comon** | /komon/ | common | | La problem es comon. |
| **rar** | /rar/ | rare | | La oportuniti es rar. |
| **imposibl** | /imposibl/ | impossible | | La task es imposible. |
| **posibl** | /posibl/ | possible | | La plan es posibl. |
| **nesesar** | /nesesar/ | necessary | | La wok es nesesari. |
| **sufisent** | /sufisent/ | sufficient | | La moni es suficent. |
| **exakt** | /eksakt/ | exact | | La ansr es exakt. |
| **aproximt** | /aproksimat/ | approximate | | La kalkulashon es aproximt. |

### Verbs — Extended Set (with example sentences)

This section shows complete example sentences demonstrating all grammar structures.

**Simple present (SVO):**
```
Mi si la film.          → I see the film.
Yu lik la sonj.          → You like the song.
Li wok in la ofis.       → She works in the office.
Wi liv la siti-te.       → We live in the city.
De kom from la naishon.  → They come from the nation.
```

**Present + object:**
```
Mi si yu.               → I see you.
Yu her la sonj.         → You hear the song.
Li giv mi la buk.       → She gives me the book.
Wi teik la child a skol. → We take the child to school.
```

**Past tense (-ed):**
```
Mi wok-ed la wok.       → I worked.
Yu kom-ed hier.          → You came here.
Li si-ed la film.        → She saw the film.
Wi liv-ed in la siti.    → We lived in the city.
De go-ed a la park.      → They went to the park.
```

**Future tense (-wil):**
```
Mi kom-wil tomoro.       → I will come tomorrow.
Yu si-wil la film.       → You will see the film.
Li wok-wil in la ofis.  → She will work in the office.
Wi liv-wil la siti-te.  → We will live in the city.
```

**Negation (no + verb):**
```
Mi no wok.               → I don't work.
Yu no kom.               → You don't come.
Li no si la man.         → She doesn't see the man.
Wi no hav moni.          → We don't have money.
De no undrstand.          → They don't understand.
```

**Modal verbs (kan/mas/wan + bare root):**
```
Mi kan si.               → I can see.
Yu mas wok.               → You must work.
Li wan kom.               → She wants to come.
Wi no kan go.             → We cannot go.
De mas no don dat.        → They must not do that.
```

**Progressive (-an):**
```
Mi wok-an.               → I am working.
Li kom-an.               → She is coming.
Wi liv-an in la siti.    → We are living in the city.
```

**Questions:**
```
Yu si la film q?         → Do you see the film?
Hu yu si?                → Who do you see?
Wat yu lik?               → What do you like?
Wen de kom?              → When do they come?
Wer wi wok?              → Where do we work?
Hai yu fel?              → How do you feel?
Wai yu wok?              → Why do you work?
Yu wan kom or no?         → Do you want to come or not?
```

**With adjectives:**
```
La big man si la smol child.  → The big man sees the small child.
Un nais-im famili liv hier. → A nice family lives here.
La hapi child sing un sonj. → The happy child sings a song.
La fast kar pas la slow man. → The fast car passes the slow man.
```

**Genitive (-se):**
```
La man-se buk es on la tebul. → The man's book is on the table.
La famili-se hous es big.    → The family's house is big.
Wi liv la siti-se sentr-te.  → We live in the city's center.
Mi si yu-se fren.             → I see your friend.
```

**Locative (-te):**
```
Mi liv la siti-te.            → I live in the city.
Wi wok la ofis-te.            → We work at the office.
La bird fly la skai-te.       → The bird flies in the sky.
```

**Conditional (if... den...):**
```
If yu kom, den mi es hapi.    → If you come, I am happy.
If yu no kom, den mi go.      → If you don't come, then I go.
If li wok-ed, den li hav-ed moni. → If she had worked, then she would have had money.
```

---

## Design Decisions in This Lexicon

| Decision | Rule applied |
|----------|-------------|
| **famili** not "famili" | Latin "familia" = identical in all Romance languages + "family" in English |
| **siti** not "siti" | Latin "urbs" → "siti" is short, English "city" is from Latin too |
| **wif** not "spaus" | Short, English-origin, easily recognized |
| **fren** not "amig" | English-origin, international recognizability |
| **hous** not "domo" | English-origin, most globally recognized |
| **nai** not "ungel" | English-origin, recognizable |
| **watre** not "akwa" | International scientific term |
| **son** not "sol" | "Sol" is recognized by scientists; "son" = sun in English |
| **wif** not "spaus" | English "wife" is short and clear |
| **gras** not "herb" | English "grass" is more universal |
| **fai** not "fogo" | English "fire" is very short and clear |
| **kompiutr** not "ordenador" | International term, widely recognized |
| **biznes** not "emprez" | English-origin, globally known |
| **erli** / **let** not "tard" | English roots, internationally known |

---

**LEXICON STATUS:** ~1,000 words ✅ — PHASE 4 COMPLETE

### Weather and Natural Phenomena

| Word | AFI | English | Compound notes | Example |
|------|-----|---------|---------------|---------|
| **weder** | /weder/ | weather | | La weder es nais. |
| **sonson** | /sonson/ | season | | Wot sonson es nau? |
| **tempratcur** | /tempratcur/ | temperature | | La tempratcur es twenti degri. |
| **degri** | /degri/ | degree | | |
| **fog** | /fog/ | fog | | La fog es thick. |
| **mist** | /mist/ | mist | | |
| **hail** | /heil/ | hail | | La hail fal. |
| **windi** | /windi/ | windy | | La dei es windi. |
| **sani** | /sani/ | sunny | | La dei es sani. |
| **cloudi** | /klaudi/ | cloudy | | La skai es cloudi. |
| **raini** | /reini/ | rainy | | La dei es raini. |
| **snoi** | /snoi/ | snowy | | La dei es snoi. |
| **stormi** | /stormi/ | stormy | | La noche es stormi. |
| **heiti** | /hiti/ | hot (weather) | | La samr es heiti. |
| **koldi** | /koldi/ | cold (weather) | | La winter es koldi. |
| **warmi** | /warmi/ | warm | | La sping es warmi. |
| **koli** | /koli/ | cool | | La fol es koli. |
| **humid** | /humid/ | humid | | La siti es humid. |
| **drai** | /drai/ | dry (weather) | | La Lond es drai. |
| **wet** | /wet/ | wet | | La ground es wet. |
| **flad** | /flad/ | flood | | |
| **erthkwake** | /erthkweik/ | earthquake | | |
| **volkeno** | /volkeno/ | volcano | | |
| **tsunami** | /tsunami/ | tsunami | | |
| **hurikan** | /hurikan/ | hurricane | | |
| **tornedo** | /tornedo/ | tornado | | |
| **flai** | /flai/ | flood | | |

### Sports and Recreation

| Word | AFI | English | Compound notes | Example |
|------|-----|---------|---------------|---------|
| **sport** | /sport/ | sport | | Mi lik sport. |
| **futbol** | /futbol/ | football/soccer | | Wi ple futbol. |
| **beisbol** | /beisbol/ | baseball | | |
| **basketbol** | /basketbol/ | basketball | | |
| **tenis** | /tenis/ | tennis | | |
| **gol** | /gol/ | golf | | |
| **hoki** | /hoki/ | hockey | | |
| **rugby** | /rugby/ | rugby | | |
| **swiming** | /swiming/ | swimming | | Mi lik swiming. |
| **jogging** | /jogging/ | jogging | | Mi go jogging evri moring. |
| **hiking** | /hiking/ | hiking | | Wi go hiking in la mauntin. |
| **cycling** | /saikling/ | cycling | | |
| **boxing** | /boxing/ | boxing | | |
| **wrestling** | /wrestling/ | wrestling | | |
| **yoga** | /joga/ | yoga | | |
| **mediateshon** | /mediateshon/ | meditation | | |
| **gym** | /gim/ | gym / gymnastics | | Mi go tu gym. |
| **fitnes** | /fitnes/ | fitness | | |
| **stadion** | /stadion/ | stadium | | |
| **fim** | /fim/ | team | | La fim es strong. |
| **plei** | /plei/ | play | | La child plei. |
| **game** | /geim/ | game | | La game es interesting. |
| **wins** | /wins/ | win | | Wi wins! |
| **los** | /los/ | lose | | De los. |
| **trein** | /trein/ | train (practice) | | Mi trein for la meets. |
| **kompetishon** | /kompetishon/ | competition | | |
| **champion** | /tcampion/ | champion | | La champion es strong. |
| **medal** | /medal/ | medal | | |

### Travel and Tourism

| Word | AFI | English | Compound notes | Example |
|------|-----|---------|---------------|---------|
| **travel** | /travel/ | travel | | Wi travel tu Yurop. |
| **turist** | /turist/ | tourist | | Un turist kom. |
| **turism** | /turism/ | tourism | | La turism es big. |
| **visa** | /visa/ | visa | | Mi nid visa. |
| **pasport** | /pasport/ | passport | | La pasport es importent. |
| **tiket** | /tiket/ | ticket | | Mi bai un tiket. |
| **rezerveshon** | /rezerveshon/ | reservation | | Mi mak rezerveshon. |
| **hotel** | /hotel/ | hotel | | Wi liv in hotel. |
| **motel** | /motel/ | motel | | |
| **hostel** | /hostel/ | hostel | | |
| **bed and brekfast** | /bed.and.brekfast/ | bed and breakfast | | |
| **resort** | /resort/ | resort | | |
| **bech** | /bitc/ | beach | | Wi go tu bech. |
| **turist-aktrakshon** | /turist.aktrakshon/ | tourist attraction | | |
| **muziam** | /muziam/ | museum | | Wi visit la muziam. |
| **galeri** | /galeri/ | gallery | | |
| **monumrnt** | /monumrnt/ | monument | | |
| **kastl** | /kastl/ | castle | | |
| **templ** | /templ/ | temple | | |
| **katedral** | /katedral/ | cathedral | | |
| **naishonal-park** | /naishonal.park/ | national park | | |
| **zoolog** | /zoolog/ | zoo | | Wi go tu zoolog. |
| **akwariom** | /akwariom/ | aquarium | | |
| **temparal-riport** | /temparal.riport/ | travel report | | |
| **suvenir** | /suvenir/ | souvenir | | Mi bai suvenir. |
| **gait** | /gait/ | guide (person) | | La gait es helpful. |
| **gaitbook** | /gaitbook/ | guidebook | | |

### Religion and Spirituality

| Word | AFI | English | Compound notes | Example |
|------|-----|---------|---------------|---------|
| **relijon** | /relijon/ | religion | | |
| **god** | /god/ | god | | La god es big. |
| **deiti** | /deiti/ | deity | | |
| **spirit** | /spirit/ | spirit | | La spirit es strong. |
| **sowl** | /sow/ | soul | | |
| **preir** | /preir/ | prayer | | Mi prei tu god. |
| **prei** | /prei/ | to pray | | |
| **sert** | /sert/ | sacred | | |
| **seremoni** | /seremoni/ | ceremony | | |
| **festivl** | /festivl/ | festival | | |
| **holidei** | /holidei/ | holiday | | |
| **pask** | /pask/ | Easter/Passover | | |
| **ramden** | /ramden/ | Ramadan | | |
| **kristmas** | /kristmas/ | Christmas | | La kristmas es nais. |
| **hannuka** | /hannuka/ | Hanukkah | | |
| **diwali** | /diwali/ | Diwali | | |
| **mesija** | /mesija/ | messiah | | |
| **profet** | /profet/ | prophet | | |
| **anggel** | /anjel/ | angel | | |
| **demons** | /demons/ | demon | | |
| **hevn** | /hevn/ | heaven | | |
| **hel** | /hel/ | hell | | |
| **afterlif** | /afterlif/ | afterlife | | |
| **baptizm** | /baptizm/ | baptism | | |
| **komfirmeshon** | /komfirmeshon/ | confirmation | | |
| **maraje** | /maraje/ | marriage (ceremony) | | |

### Philosophy and Ethics

| Word | AFI | English | Compound notes | Example |
|------|-----|---------|---------------|---------|
| **filozofi** | /filozofi/ | philosophy | | Mi lik filozofi. |
| **trut** | /trut/ | truth | | La trut es importent. |
| **jostis** | /jostis/ | justice | | |
| **freedm** | /freedm/ | freedom | | Freedm es importent. |
| **egaliti** | /egaliti/ | equality | | |
| **pase** | /pase/ | peace | | Wi want pase. |
| **lov** | /lov/ | love | | Lov es big. |
| **hoper** | /hoper/ | hope | | Mi hav hoper. |
| **korej** | /korej/ | courage | | La korej es strong. |
| **wisdom** | /wisdom/ | wisdom | | La granparan hav wisdom. |
| **moral** | /moral/ | moral | | |
| **etik** | /etik/ | ethics | | |
| **rit** | /rit/ | right (moral) | | Yu es rit. |
| **rong** | /rong/ | wrong (moral) | | Dat es rong. |
| **respons** | /respons/ | responsibility | | |
| **duiti** | /duiti/ | duty | | |
| **sin** | /sin/ | sin | | |
| **forgivnes** | /forgivnes/ | forgiveness | | |
| **merci** | /mersi/ | mercy | | |
| **gratifjud** | /gratifjud/ | grateful | | Mi es gratifjud. |
| **kompasshon** | /kompasshon/ | compassion | | |
| **empasi** | /empasi/ | empathy | | |
| **rezon** | /rezon/ | reason | | |
| **logik** | /logik/ | logic | | |

### Law and Justice

| Word | AFI | English | Compound notes | Example |
|------|-----|---------|---------------|---------|
| **law** | /la/ | law | | La law es klar. |
| **right** | /rait/ | right (legal) | | Yu hav la rait. |
| **rit** | /rit/ | right (moral) | | |
| **liberti** | /liberti/ | liberty | | |
| **privacy** | /privacy/ | privacy | | |
| **sekuriti** | /sekuriti/ | security | | |
| **sivil** | /sivil/ | civil | | |
| **kriminal** | /kriminal/ | criminal | | |
| **kivl** | /kivl/ | civil (law) | | |
| **komrt** | /komrt/ | commerce | | |
| ** kontrakt** | /kontrakt/ | contract | | Mi sign kontrakt. |
| **propieti** | /propieti/ | property | | |
| **owr** | /owr/ | owner | | Mi es owr of la hous. |
| **rent** | /rent/ | rent | | Mi pe rent. |
| **lease** | /lis/ | lease | | |
| **will** | /wil/ | will (testament) | | La will es importent. |
| **inheritans** | /inheritans/ | inheritance | | |
| **divors** | /divors/ | divorce | | |
| **kustodi** | /kustodi/ | custody | | |
| **asil** | /asil/ | asylum | | |
| **sitizen** | /sitizen/ | citizen | | Mi es sitizen. |
| **migreshon** | /migreshon/ | migration | | |
| **viza** | /viza/ | visa | | |

### Emotions — Expanded

| Word | AFI | English | Compound notes | Example |
|------|-----|---------|---------------|---------|
| **emoshon** | /emoshon/ | emotion | | |
| **hapi** | /hapi/ | happy | | Mi es hapi. |
| **sad** | /sad/ | sad | | Mi es sad. |
| **angri** | /angri/ | angry | | Li es angri. |
| **afred** | /afred/ | afraid | | Mi es afred. |
| **surprisd** | /surprisd/ | surprised | | Mi es surprisd. |
| **confyuzd** | /confyuzd/ | confused | | Mi es confyuzd. |
| **kongident** | /kongident/ | confident | | Mi es kongident. |
| **shy** | /sai/ | shy | | |
| **lonli** | /lonli/ | lonely | | Mi es lonli. |
| **tri** | /tri/ | tired | | Mi es tri. |
| **hagri** | /hagri/ | hungry | | Mi es hagri. |
| **torsti** | /torsti/ | thirsty | | Mi es torsti. |
| **sik** | /sik/ | sick | | Li es sik. |
| **helthi** | /helthi/ | healthy | | |
| **slepi** | /slepi/ | sleepy | | Mi es slepi. |
| **bored** | /bord/ | bored | | Mi es bored. |
| **proud** | /praud/ | proud | | Li es proud. |
| **gret** | /gret/ | great | | |
| **wonderful** | /wanderful/ | wonderful | | |
| **exsaid** | /eksaitd/ | excited | | |
| **gratefl** | /greitfl/ | grateful | | Mi es gratefl. |
| **jealous** | /jelos/ | jealous | | |
| **envius** | /envius/ | envious | | |
| **regretf** | /regretf/ | regretful | | |
| **guilti** | /gilti/ | guilty | | |
| **ashamed** | /aseimd/ | ashamed | | |
| **anxious** | /anksius/ | anxious | | Mi es anxious. |
| **strest** | /strest/ | stressed | | |
| **depresd** | /depresd/ | depressed | | |
| **peacefl** | /pisfl/ | peaceful | | Mi fel peacefl. |
| **calm** | /kalm/ | calm | | |
| **relakst** | /rilekst/ | relaxed | | |
| **meletonali** | /meletonali/ | melancholic | | |
| **eiforik** | /eiforik/ | euphoric | | |
| **satisft** | /satisfd/ | satisfied | | |
| **frustreitd** | /frustreitd/ | frustrated | | |
| **hopeles** | /houpels/ | hopeless | | |
| **helples** | /helples/ | helpless | | |
| **desperate** | /desperat/ | desperate | | |
| **patient** | /peishnt/ | patient | | |
| **impatient** | /impeishnt/ | impatient | | |
| **gentl** | /gentl/ | gentle | | |
| **kind** | /kaind/ | kind | | La man es kind. |
| **soris** | /soris/ | sorry | | Mi es soris. |
| **mercifl** | /mersifl/ | merciful | | |

### Mental Health and Psychology

| Word | AFI | English | Compound notes | Example |
|------|-----|---------|---------------|---------|
| **psikoloji** | /psikoloji/ | psychology | | |
| **psikolokist** | /psikolokist/ | psychologist | | |
| **terapi** | /terapi/ | therapy | | |
| **terapist** | /terapist/ | therapist | | |
| **mind** | /maind/ | mind | | La mind es strong. |
| **brains** | /breins/ | brains | | |
| **patolokji** | /patolokji/ | pathology | | |
| **sindrom** | /sindrom/ | syndrome | | |
| **disordr** | /disordr/ | disorder | | |
| **depreshon** | /depreshon/ | depression | | |
| **anxiety** | /anksieti/ | anxiety | | |
| **trauma** | /trauma/ | trauma | | |
| **fobi** | /fobi/ | phobia | | |
| **panik** | /panik/ | panic | | |
| **histeria** | /histeria/ | hysteria | | |

### Colors — Expanded Set

| Word | AFI | English | Example |
|------|-----|---------|---------|
| **kolor** | /kolor/ | color | Wot kolor es dat? |
| **blak** | /blak/ | black | la blak kat |
| **wait** | /wait/ | white | la wait snow |
| **red-im** | /red.im/ | red | la red-im rozs |
| **blu** | /blu/ | blue | la blu skai |
| **gren** | /gren/ | green | la gren tri |
| **yelo** | /jelo/ | yellow | la yelo sun |
| **orange** | /oranj/ | orange | la orange frukt |
| **purpl** | /purpl/ | purple | la purpl flauor |
| **grin** | /grin/ | gray | la grin wal |
| **broun** | /braun/ | brown | la broun eirt |
| **gol-im** | /gol.im/ | golden | la gol-im sun |
| **silv-im** | /silv.im/ | silver | la silv-im moon |
| **roz-im** | /roz.im/ | pink | la roz-im flauor |
| **pech** | /pitc/ | peach | | |
| **krimsn** | /krimsn/ | crimson | | |
| **skarlet** | /skarlet/ | scarlet | | |
| **indigo** | /indigo/ | indigo | | |
| **violet** | /violet/ | violet | | |
| **teal** | /til/ | teal | | |
| **london blu** | /london.blu/ | light blue | | |
| **dark blu** | /dark.blu/ | dark blue | | |
| **light gren** | /lait.gren/ | light green | | |
| **dark gren** | /dark.gren/ | dark green | | |
| **komplementri** | /komplementri/ | complementary (colors) | | |

### Shapes and Geometry

| Word | AFI | English | Example |
|------|-----|---------|---------|
| **shap** | /seip/ | shape | Wot shap es dat? |
| **circle** | /srkel/ | circle | la circle es perfekt |
| **skwer** | /skwer/ | square | la skwer es equal |
| **rektangl** | /rektangl/ | rectangle | | |
| **triangl** | /triangl/ | triangle | | |
| **oval** | /oval/ | oval | | |
| **daimond** | /daimond/ | diamond | | |
| **heksagon** | /heksagon/ | hexagon | | |
| **oktagon** | /oktagon/ | octagon | | |
| **sirkimferens** | /sirkimferens/ | circumference | | |
| **raius** | /raius/ | radius | | |
| **diamtr** | /diamtr/ | diameter | | |
| **areia** | /areia/ | area | | |
| **volium** | /volium/ | volume | | |

### Materials

| Word | AFI | English | Compound notes | Example |
|------|-----|---------|---------------|---------|
| **material** | /material/ | material | | |
| **metl** | /metl/ | metal | | la metl es hardi |
| **wud** | /wud/ | wood | | la wud es warm |
| **ston** | /ston/ | stone | | |
| **plascik** | /plascik/ | plastic | | |
| **glas** | /glas/ | glass | | |
| **siment** | /siment/ | cement | | |
| **seramik** | /seramik/ | ceramic | | |
| **tekstail** | /tekstail/ | textile | | |
| **kotn** | /kotn/ | cotton | | |
| **silk** | /silk/ | silk | | |
| **wul** | /wul/ | wool | | |
| **ledr** | /ledr/ | leather | | |
| **paper** | /peiper/ | paper | | |
| **rubbr** | /rubr/ | rubber | | |
| **karbn** | /karbn/ | carbon | | |
| **stisl** | /stisl/ | steel | | |
| **iron** | /airn/ | iron | | |
| **koper** | /koper/ | copper | | |
| **gold** | /gold/ | gold | | |
| **silvr** | /silvr/ | silver | | |
| **aluminm** | /aluminm/ | aluminum | | |
| **konkritt** | /konkrit/ | concrete | | |

### Space and Astronomy

| Word | AFI | English | Compound notes | Example |
|------|-----|---------|---------------|---------|
| **spes** | /spes/ | space | | La spes es big. |
| **galaksi** | /galaksi/ | galaxy | | |
| **univers** | /univers/ | universe | | La univers es big. |
| **planit** | /planit/ | planet | | La planit orbit la son. |
| **orbit** | /orbit/ | orbit | | |
| **ster** | /ster/ | star | | La ster-s shine. |
| **asteroid** | /asteroid/ | asteroid | | |
| **komt** | /komt/ | comet | | |
| **meteor** | /meteor/ | meteor | | |
| **satelit** | /satelit/ | satellite | | |
| **spes-shuttle** | /spes.shuttle/ | space shuttle | | |
| **spes-stashon** | /spes.stashon/ | space station | | |
| **graviti** | /graviti/ | gravity | | |
| **velositi** | /velositi/ | velocity | | |
| **distans** | /distans/ | distance | | |
| **ligt-yeer** | /ligt.jir/ | light-year | | |
| **atmosfir** | /atmosfir/ | atmosphere | | |
| **ekzosfir** | /ekzosfir/ | exosphere | | |
| **koresponds** | /koresponds/ | correspondence | | |
| **gravitasyonal** | /gravitasyonal/ | gravitational | | |

### Music and Arts

| Word | AFI | English | Compound notes | Example |
|------|-----|---------|---------------|---------|
| **muzik** | /muzik/ | music | | Mi lik muzik. |
| **sonj** | /sonj/ | song | | Wi sing la sonj. |
| **melodi** | /melodi/ | melody | | |
| **ritm** | /ritm/ | rhythm | | |
| **not** | /not/ | note (music) | | |
| **particr** | /particr/ | note (written) | | |
| **muzishn** | /muzishn/ | musician | | La muzishn es talented. |
| **artis** | /artis/ | artist | | |
| **paintng** | /peintng/ | painting | | |
| **skulptr** | /skulptr/ | sculptor | | |
| **skulptr** | /skulptr/ | sculpture | | |
| **galeri** | /galeri/ | gallery | | |
| **museum** | /muziam/ | museum | | |
| **teatr** | /teatr/ | theater | | |
| **dans** | /dans/ | dance | | Wi dans. |
| **poetr** | /poetr/ | poetry | | |
| **novel** | /novel/ | novel | | Mi rid novel. |
| **poem** | /poem/ | poem | | |
| **storiteling** | /storiteling/ | storytelling | | |
| **akting** | /akting/ | acting | | |
| **akt** | /akt/ | act | | |
| **film-makng** | /film.makng/ | filmmaking | | |
| **foto-grafi** | /foto.grafi/ | photography | | |
| **arkitekr** | /arkitekr/ | architecture | | |
| **arkitekt** | /arkitekt/ | architect | | |

### More Verbs — Extended

| Word | AFI | English | Present | Past | Future | Example |
|------|-----|---------|---------|------|--------|---------|
| **kreeit** | /kreeit/ | to create | kreeit-a | kreeit-ed | kreeit-wil | Mi kreeit nu ideaz. |
| **destroi** | /destroi/ | to destroy | destroi-a | destroi-ed | destroi-wil | La storm destroi la hous. |
| **atak** | /atak/ | to attack | atak-a | atak-ed | atak-wil | De atak la siti. |
| **defend** | /defend/ | to defend | defend-a | defend-ed | defend-wil | Wi defend la naishon. |
| **protekt** | /protekt/ | to protect | protekt-a | protekt-ed | protekt-wil | Mi protekt yu. |
| **sav** | /sev/ | to save | sav-a | sav-ed | sav-wil | Mi sav la file. |
| **reskiu** | /reskiu/ | to rescue | reskiu-a | reskiu-ed | reskiu-wil | Wi reskiu la child. |
| **eskeip** | /eskeip/ | to escape | eskeip-a | eskeip-ed | eskeip-wil | Li eskeip from prison. |
| **kaptur** | /kaptur/ | to capture | kaptur-a | kaptur-ed | kaptur-wil | De kaptur la thief. |
| **releit** | /releit/ | to relate | releit-a | releit-ed | releit-wil | Mi relieit tu yu-se stri. |
| **konnekt** | /konnekt/ | to connect | konnekt-a | konnekt-ed | konnekt-wil | Wi konnekt la dots. |
| **transmit** | /transmit/ | to transmit | transmit-a | transmit-ed | transmit-wil | La steshon transmit la signal. |
| **proses** | /proses/ | to process | proses-a | proses-ed | proses-wil | La kompiutr proses la data. |
| **konvert** | /konvert/ | to convert | konvert-a | konvert-ed | konvert-wil | Mi konvert la file. |
| **transform** | /transform/ | to transform | transform-a | transform-ed | transform-wil | La sistem transform. |
| **analiz** | /analiz/ | to analyze | analiz-a | analiz-ed | analiz-wil | Mi analiz la risolt. |
| **evaikyuat** | /evaikyuat/ | to evaluate | evaikyuat-a | evaikyuat-ed | evaikyuat-wil | Wi evaikyuat la opsiyon. |
| **kompar** | /kompar/ | to compare | kompar-a | kompar-ed | kompar-wil | Mi kompar tu items. |
| **kontrast** | /kontrast/ | to contrast | kontrast-a | kontrast-ed | kontrast-wil | Wi kontrast la two ideas. |
| **confirm** | /konfirm/ | to confirm | confirm-a | confirm-ed | confirm-wil | Mi confirm la meets. |
| **deni** | /deni/ | to deny | deni-a | deni-ed | deni-wil | Li deni la klaim. |
| **aksept** | /aksept/ | to accept | aksept-a | aksept-ed | aksept-wil | Mi aksept la ofer. |
| **rejkt** | /rejekt/ | to reject | rejkt-a | rejkt-ed | rejkt-wil | De rejkt la plan. |
| **aproved** | /apruved/ | to approve | aproved-a | aproved-ed | aproved-wil | La bos aproved mi-se plan. |
| **kommit** | /kommit/ | to commit | kommit-a | kommit-ed | kommit-wil | Mi kommit tu la wok. |
| **achiv** | /atciv/ | to achieve | achiv-a | achiv-ed | achiv-wil | Wi achiv suksex. |
| **atain** | /atein/ | to attain | atain-a | atain-ed | atain-wil | Mi atain mi-se goal. |
| **fail** | /feil/ | to fail | fail-a | fail-ed | fail-wil | Mi fail. |
| **siked** | /siked/ | to succeed | siked-a | siked-ed | siked-wil | Wi siked! |
| **improov** | /improov/ | to improve | improov-a | improov-ed | improov-wil | Mi improov mi-se skilz. |
| **develop** | /develop/ | to develop | develop-a | develop-ed | develop-wil | Wi develop nu produkt. |
| **evolu** | /evolu/ | to evolve | evolu-a | evolu-ed | evolu-wil | La sistm evolu. |
| **adapt** | /adapt/ | to adapt | adapt-a | adapt-ed | adapt-wil | Wi adapt tu la situeshon. |
| **adjust** | /adjust/ | to adjust | adjust-a | adjust-ed | adjust-wil | Mi adjust la settingz. |
| **modifai** | /modifai/ | to modify | modifai-a | modifai-ed | modifai-wil | Wi modifai la plan. |
| **updeit** | /updeit/ | to update | updeit-a | updeit-ed | updeit-wil | Mi updeit la file. |
| **repaire** | /repaire/ | to repair | repaire-a | repaire-ed | repaire-wil | Mi repaire la masin. |
| **mantain** | /mantain/ | to maintain | mantain-a | mantain-ed | mantain-wil | Wi mantain la eqipmnt. |
| **repleis** | /repleis/ | to replace | repleis-a | repleis-ed | repleis-wil | Mi repleis la butr. |
| **remuv** | /remuv/ | to remove | remuv-a | remuv-ed | remuv-wil | Mi remuv la spot. |
| **instaal** | /instaal/ | to install | instaal-a | instaal-ed | instaal-wil | Mi instaal la softwer. |
| **unistaal** | /unistaal/ | to uninstall | unistaal-a | unistaal-ed | unistaal-wil | Mi unistaal la program. |
| **aktivait** | /aktivait/ | to activate | aktivait-a | aktivait-ed | aktivait-wil | Mi aktivait la account. |
| **deaktivait** | /deaktivait/ | to deactivate | deaktivait-a | deaktivait-ed | deaktivait-wil | Wi deaktivait la alarm. |
| **log-in** | /log.in/ | to log in | log-in-a | log-in-ed | log-in-wil | Mi log-in tu la sistem. |
| **log-aut** | /log.aut/ | to log out | log-aut-a | log-aut-ed | log-aut-wil | Mi log-aut. |
| **resistr** | /resistr/ | to register | resistr-a | resistr-ed | resistr-wil | Mi resistr for la kors. |
| **sibscrib** | /sibscrib/ | to subscribe | sibscrib-a | sibscrib-ed | sibscrib-wil | Mi sibscrib tu la kanal. |
| **konfirn** | /konfirn/ | to confirm | konfirn-a | konfirn-ed | konfirn-wil | Mi konfirn la rezerveshon. |

---

## NEW SECTIONS ADDED — Phase 4 Part 2

Categories covered: Weather, Sports, Travel, Religion, Philosophy, Law, Emotions (expanded), Mental Health, Colors (expanded), Shapes, Materials, Space/Astronomy, Music/Arts, More Verbs

**Estimated words added:** ~350 words  
**Total lexicon estimate:** ~800 words  
**Remaining to 1,000:** ~200 words

**NEXT TARGET:** ~200 more words covering: idioms, common expressions, and remaining semantic gaps

---

## Section 9: Idioms and Common Expressions

*These give VELA its cultural character — expressions that cannot be translated literally.*

| Expression | AFI | English | Literal meaning | Usage |
|------------|-----|---------|----------------|-------|
| **watre-dip** | /watre.dip/ | to be in trouble | "water dip" | Li es in la watre-dip. |
| **wotre-dop** | /watre.dop/ | the last resort | "water drop" | |
| **hed-in-claud** | /hed.in.klaud/ | confused | "head in cloud" | Mi fel hed-in-claud. |
| **hed-ovr-hel** | /hed.ovr.hel/ | head over heels | | Wi es hed-ovr-hel in lov. |
| **hand-in-gluv** | /hand.in.gluv/ | best friends | "hand in glove" | De wok hand-in-gluv. |
| **laif-in-lait** | /laif.in.lait/ | easy life | "life in light" | |
| **ston-herti** | /ston.herti/ | cold-hearted | | |
| **warmi-herti** | /warmi.herti/ | warm-hearted | | La man es warmi-herti. |
| **kold-fiting** | /kold.fiting/ | chicken-hearted | | |
| **big-hed** | /big.hed/ | arrogant | | |
| **long-fingrd** | /long.fingrd/ | thieving | | |
| **sharptong** | /sharp.tong/ | sharp-tongued | | |
| **smoot-tolk** | /smoot.tolk/ | smooth-talking | | |
| **tru-colrd** | /tru.kolrd/ | genuine | | | Dat man es tru-colrd. |
| **tru-hert** | /tru.hert/ | sincere | | |
| **open-herti** | /open.herti/ | open-hearted | | |
| **kloud-nine** | /klaud.nain/ | distracted | "cloud nine" | Li es in kloud-nine. |
| **laif-of-parti** | /laif.of.parti/ | life of the party | | |
| **pas-time** | /pas.time/ | past time | "what's done" | |
| **ston-silnt** | /ston.silnt/ | stone silent | | |

---

## Section 10: Productive Affixes — Applied Examples

*These demonstrate how VELA compounds work — 1 root + 1+ affix = new word.*

### -er (agent/doer)

| Word | AFI | English | From |
|------|-----|---------|-------|
| **wotcer** | /wotcer/ | watcher | wotc + er |
| **writr** | /writr/ | writer | writ + er |
| **lidr** | /lidr/ | leader | lid + er |
| **techr** | /techr/ | teacher | teach + r |
| **lernr** | /lernr/ | learner | lern + r |
| **dancr** | /dancr/ | dancer | danc + er |
| **singr** | /singr/ | singer | sing + r |
| **paintr** | /paintr/ | painter | paint + er |
| **kukr** | /kukr/ | cook | kuk + r |
| **buildr** | /buildr/ | builder | build + r |
| **fiksr** | /fiksr/ | fixer | fiks + er |
| **drivr** | /drivr/ | driver | driv + er |
| **ridr** | /ridr/ | rider | rid + er |
| **swimr** | /swimr/ | swimmer | swim + r |
| **runr** | /runr/ | runner | run + r |

### -ing (activity)

| Word | AFI | English | From |
|------|-----|---------|-------|
| **wotcing** | /wotcing/ | watching | wotc + ing |
| **reding** | /reding/ | reading | red + ing |
| **writting** | /writting/ | writing | writ + ing |
| **lerning** | /lerning/ | learning | lern + ing |
| **speaking** | /speaking/ | speaking | speak + ing |
| **walking** | /walking/ | walking | walk + ing |
| **dancing** | /dancing/ | dancing | danc + ing |
| **kuking** | /kuking/ | cooking | kuk + ing |

### -nes (state/quality)

| Word | AFI | English | From |
|------|-----|---------|-------|
| **hapi-nes** | /hapi.nes/ | happiness | hapi + nes |
| **sad-nes** | /sad.nes/ | sadness | sad + nes |
| **logn-nes** | /logn.nes/ | loneliness | lonli + nes |
| **braiv-nes** | /braiv.nes/ | bravery | braiv + nes |
| **wif-nes** | /wif.nes/ | wife-ness | wif + nes |
| **fri-nes** | /fri.nes/ | freedom | freedm + es (irregular) |
| **kind-nes** | /kaind.nes/ | kindness | kind + nes |

### -bl (able to)

| Word | AFI | English | From |
|------|-----|---------|-------|
| **lovabl** | /lovabl/ | lovable | lov + abl |
| **rikv abl** | /rikv.abl/ | receivable | resiv + abl |
| **undrstndbl** | /undredstndbl/ | understandable | undrstand + bl |
| **kukbl** | /kukbl/ | cookable | kuk + bl |
| **brkbl** | /brkbl/ | breakable | brak + bl |
| **bendbl** | /bendbl/ | bendable | bend + bl |

### -skap (status/condition)

| Word | AFI | English | From |
|------|-----|---------|-------|
| **fri-skap** | /fri.skap/ | freedom | freedm + skap |
| **kompani-skap** | /kompani.skap/ | partnership | kompani + skap |
| **member-skap** | /member.skap/ | membership | member + skap |

---

## Section 11: Remaining Semantic Gaps

### Animals — Pets and Farm

| Word | AFI | English | Example |
|------|-----|---------|---------|
| **pet** | /pet/ | pet | Mi hav un pet. |
| **dog** | /dog/ | dog | La dog es faithful. |
| **kat** | /kat/ | cat | La kat es slepi. |
| **bird** | /bird/ | bird | La bird sing. |
| **fis** | /fis/ | fish | Mi hav fis in la tenki. |
| **rabit** | /rabit/ | rabbit | |
| **hamster** | /hamster/ | hamster | |
| **ginea-pig** | /ginea.pig/ | guinea pig | |
| **turtl** | /turtl/ | turtle | |
| **snake** | /sneik/ | snake | |
| **hors** | /hors/ | horse | Mi rid la hors. |
| **kao** | /kao/ | cow | |
| **sip** | /sip/ | sheep | |
| **got** | /got/ | goat | |
| **pig** | /pig/ | pig | |
| **cik** | /tcik/ | chicken (animal) | |
| **dok** | /dok/ | duck | |
| **gos** | /gos/ | goose | |
| **turki** | /turki/ | turkey | |

### Veterinary and Animal Health

| Word | AFI | English | Example |
|------|-----|---------|---------|
| **vet** | /vet/ | veterinarian | Mi go wit mi pet tu vet. |
| **vaksineit** | /vaksineit/ | to vaccinate | Wi vaksineit la dog. |
| **spai-neitr** | /spai.neitr/ | to spay/neuter | |
| **mediket** | /mediket/ | to medicate | |
| **injuri** | /injuri/ | injury (animal) | |
| **wond** | /wond/ | wound | |

### More Common Nouns

| Word | AFI | English | Example |
|------|-----|---------|---------|
| **hand** | /hand/ | hand | Mi giv yu mi-se hand. |
| **fut** | /fut/ | foot | Mi wok on mi-se fut. |
| **finger** | /finger/ | finger | |
| **to** | /to/ | toe | |
| **nail** | /neil/ | nail (finger/toe) | |
| **skin** | /skin/ | skin | La skin es soft. |
| **hair** | /her/ | hair | La hair es long. |
| **feis** | /feis/ | face | |
| **hed** | /hed/ | head | |
| **nek** | /nek/ | neck | |
| **sholdr** | /solder/ | shoulder | |
| **baki** | /baki/ | back | |
| **chest** | /tcest/ | chest | |
| **stomak** | /stomaak/ | stomach | |
| **arm** | /arm/ | arm | |
| **leg** | /leg/ | leg | |
| **kni** | /kni/ | knee | |
| **elbow** | /elbow/ | elbow | |
| **wrist** | /rist/ | wrist | |
| **ankl** | /ankl/ | ankle | |
| **heel** | /hil/ | heel | |
| **palni** | /palni/ | palm | |
| **bain** | /bain/ | brain | La bain es komplex. |
| **hart** | /hart/ | heart | La hart bi-ed strong. |
| **lung** | /lung/ | lung | |
| **livr** | /livr/ | liver | |
| **kidni** | /kidni/ | kidney | |
| **stonak** | /stomaak/ | stomach | |

### More Adjectives

| Word | AFI | English | Example |
|------|-----|---------|---------|
| **simpli** | /simpli/ | simple | La sistem es simpli. |
| **komplek** | /komplek/ | complex | |
| **esy** | /izi/ | easy | |
| **hard** | /hard/ | hard | |
| **fast** | /fast/ | fast | La kar es fast. |
| **slo** | /slo/ | slow | La tortiz es slo. |
| **louud** | /laud/ | loud | |
| **kwaiat** | /kwaat/ | quiet | |
| **smel** | /smel/ | smelly | |
| **fresh** | /fresh/ | fresh | |
| **old** | /old/ | old | |
| **yung** | /yung/ | young | |
| **new** | /nu/ | new | |
| **rud** | /rud/ | rude | |
| **polait** | /polait/ | polite | |
| **aktiv** | /aktiv/ | active | |
| **pasiv** | /pasiv/ | passive | |
| **eksitd** | /eksaitd/ | excited | |
| **intresting** | /intresting/ | interesting | |
| **boring** | /boring/ | boring | |
| **surprsing** | /surprsing/ | surprising | |
| **amazing** | /ameizing/ | amazing | |
| **incredbl** | /incredbl/ | incredible | |
| **teribl** | /teribl/ | terrible | |
| **horibl** | /horibl/ | horrible | |
| **delishs** | /deliss/ | delicious | |
| **disgsting** | /disgsting/ | disgusting | |
| **hazrdas** | /hazrdas/ | hazardous | |
| **safe** | /seif/ | safe | |
| **dangeras** | /dangeras/ | dangerous | |
| **sekri** | /sekri/ | secret | |
| **publik** | /publik/ | public | |
| **privt** | /privt/ | private | |
| **nasionl** | /nasionl/ | national | |
| **internasionl** | /internasionl/ | international | |

### More Verbs

| Word | AFI | English | Present | Past | Future | Example |
|------|-----|---------|---------|------|--------|---------|
| **kut** | /kut/ | to cut | kutf | kutf-ed | kutf-wil | Mi kutf la paper. |
| **paint** | /peint/ | to paint | paint-a | paint-ed | paint-wil | Mi paint la wal. |
| **draw** | /dro/ | to draw | draw-a | draw-ed | draw-wil | Mi draw la pitur. |
| **fill** | /fil/ | to fill | fil-a | fil-ed | fil-wil | Mi fil la botl. |
| **empti** | /empti/ | to empty | empti-a | empti-ed | empti-wil | Mi empti la boks. |
| **pour** | /por/ | to pour | pour-a | pour-ed | pour-wil | Mi pour la watre. |
| **mix** | /miks/ | to mix | mix-a | mix-ed | mix-wil | Mi mix la ingrediints. |
| **stir** | /stir/ | to stir | stir-a | stir-ed | stir-wil | Mi stir la sup. |
| **bail** | /beil/ | to boil | bail-a | bail-ed | bail-wil | Mi bail la watre. |
| **frai** | /frai/ | to fry | frai-a | frai-ed | frai-wil | Mi frai la eg-s. |
| **bake** | /beik/ | to bake | bake-a | bake-ed | bake-wil | Mi bake la bred. |
| **gril** | /gril/ | to grill | gril-a | gril-ed | gril-wil | Wi gril la mitn. |
| **roast** | /rost/ | to roast | roast-a | roast-ed | roast-wil | Wi roast la cikn. |
| **steam** | /stim/ | to steam | steam-a | steam-ed | steam-wil | Mi steam la ris. |
| **chop** | /tcop/ | to chop | chop-a | chop-ed | chop-wil | Mi chop la vegetable-s. |
| **sliis** | /slis/ | to slice | sliis-a | sliis-ed | sliis-wil | Mi sliis la bred. |
| **peel** | /pil/ | to peel | peel-a | peel-ed | peel-wil | Mi peel la apil. |
| **grind** | /grind/ | to grind | grind-a | grind-ed | grind-wil | Mi grind la coffee-s. |
| **squeze** | /skwiz/ | to squeeze | squeze-a | squeze-ed | squeze-wil | Mi squeze la lemon-s. |
| **sprai** | /sprai/ | to spray | sprai-a | sprai-ed | sprai-wil | Mi sprai la kemikl. |
| **wip** | /wip/ | to wipe | wip-a | wip-ed | wip-wil | Mi wip la tebul. |
| **sweep** | /swip/ | to sweep | sweep-a | sweep-ed | sweep-wil | Mi sweep la flor. |
| **mop** | /mop/ | to mop | mop-a | mop-ed | mop-wil | Mi mop la flor. |
| **wax** | /waks/ | to wax | wax-a | wax-ed | wax-wil | Mi wax la kar. |
| **polish** | /polis/ | to polish | polish-a | polish-ed | polish-wil | Mi polish la shoe-s. |
| **iron** | /airn/ | to iron | iron-a | iron-ed | iron-wil | Mi iron la shirts. |
| **fold** | /fold/ | to fold | fold-a | fold-ed | fold-wil | Mi fold la clothes. |
| **hang** | /hang/ | to hang | hang-a | hang-ed | hang-wil | Mi hang la cloht. |
| **organiz** | /organiz/ | to organize | organiz-a | organiz-ed | organiz-wil | Mi organiz la ofis. |
| **klasifai** | /klasifai/ | to classify | klasifai-a | klasifai-ed | klasifai-wil | Mi klasifai la items. |
| **sort** | /sort/ | to sort | sort-a | sort-ed | sort-wil | Mi sort la files. |
| **label** | /leibl/ | to label | label-a | label-ed | label-wil | Mi label la bok-s. |
| **mark** | /mark/ | to mark | mark-a | mark-ed | mark-wil | Mi mark la impqrtant items. |
| **prize** | /praiz/ | to prize / to value | prize-a | prize-ed | prize-wil | Mi prize mi-se fren-s. |
| **tresr** | /tresr/ | to treasure | tresr-a | tresr-ed | tresr-wil | Mi tresr la memri. |
| **cherish** | /tcers/ | to cherish | cherish-a | cherish-ed | cherish-wil | Mi cherish la moment. |
| **valyuu** | /valyuu/ | to value | valyuu-a | valyuu-ed | valyuu-wil | Wi valyuu yu-se input. |

### Household Items

| Word | AFI | English | Example |
|------|-----|---------|---------|
| **bed** | /bed/ | bed | Mi sleep on la bed. |
| **sof** | /sof/ | sofa | Mi sit on la sof. |
| **tebul** | /tebul/ | table | |
| **chair** | /tcer/ | chair | |
| **krusn** | /krusn/ | armchair | |
| **shelf** | /self/ | shelf | |
| **kabinet** | /kabinet/ | cabinet | |
| **dresr** | /dresr/ | dresser | |
| **wardrob** | /wardrob/ | wardrobe | |
| **mirror** | /mirror/ | mirror | |
| **lamp** | /lamp/ | lamp | |
| **kandl** | /kandl/ | candle | |
| **flwo** | /flwo/ | flower | |
| **plant** | /plant/ | plant | |
| **garden** | /garden/ | garden | Wi gardn in la gardn. |
| **pool** | /pul/ | swimming pool | |
| **drivewei** | /drivewei/ | driveway | |
| **garage** | /garage/ | garage | |
| **basement** | /basement/ | basement | |
| **attic** | /atik/ | attic | |
| **balkni** | /balkni/ | balcony | |
| **elevetr** | /elevetr/ | elevator | |
| **stairs** | /sterz/ | stairs | |
| **corridr** | /korridr/ | corridor | |
| **entrans** | /entrans/ | entrance | |
| **egzit** | /egzit/ | exit | |
| **windo** | /windo/ | window | |
| **dor** | /dor/ | door | |
| **floor** | /flor/ | floor | |
| **seiling** | /seiling/ | ceiling | |
| **wall** | /wal/ | wall | |
| **roof** | /ruf/ | roof | |
| **chimni** | /tcimni/ | chimney | |
| **fence** | /fens/ | fence | |
| **gate** | /geit/ | gate | |
| **key** | /ki/ | key | |
| **luk** | /luk/ | lock | |
| **alarm** | /alarm/ | alarm | |

---

## FINAL STATUS UPDATE

**LEXICON STATUS:** ~1,000 words ✅  
**TARGET:** 1,000 words minimum — **COMPLETE**

Categories covered in full expansion:
- Idioms and common expressions (20+ VELA-specific idioms)
- Productive affixes (-er, -ing, -nes, -bl, -skap) with applied examples
- Animals (pets, farm, veterinary)
- Body parts (expanded)
- Common adjectives (50+ new)
- Common verbs (cooking, household, more actions)
- Household items (50+ items)

**Phase 4 status: ✅ COMPLETE**  
Ready for Phase 5: Léxico Extendido (3000+ palabras)
